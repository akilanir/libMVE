...
/* loaded from: kore-javamail-1.0.2.jar:com/sun/mail/smtp/SMTPTransport.class */
public class SMTPTransport extends Transport {
    ...
    protected synchronized boolean protocolConnect(String host, int port, String user, String passwd) throws MessagingException {
        ...
        try {
            ...
            boolean connected2 = authenticate(user, passwd);
            if (!connected2) {
                try {
                    closeConnection();
    ...
    private boolean authenticate(String user, String passwd) throws MessagingException {
        ...
        while (st.hasMoreTokens()) {
            ...
            if (disabled) {
                ...
                String m2 = m.toUpperCase(Locale.ENGLISH);
                if (!supportsAuthentication(m2)) {
                    this.logger.log(Level.FINE, "mechanism {0} not supported by server", m2);
                } else {
                    Authenticator a = (Authenticator) this.authenticators.get(m2);
                    if (a == null) {
                        this.logger.log(Level.FINE, "no authenticator for mechanism {0}", m2);
                    } else {
    ...
    /* loaded from: kore-javamail-1.0.2.jar:com/sun/mail/smtp/SMTPTransport$Authenticator.class */
                ...
                try {
                    ...
                    if (t != null) {
                        ...
                        if (t instanceof Exception) {
                            throw new AuthenticationFailedException(SMTPTransport.this.getLastServerResponse(), (Exception) t);
                        }
                        if (!$assertionsDisabled) {
                            throw new AssertionError("unknown Throwable");
                        }
    ...
    /* loaded from: kore-javamail-1.0.2.jar:com/sun/mail/smtp/SMTPTransport$LoginAuthenticator.class */
    ...
    /* loaded from: kore-javamail-1.0.2.jar:com/sun/mail/smtp/SMTPTransport$PlainAuthenticator.class */
    ...
    /* loaded from: kore-javamail-1.0.2.jar:com/sun/mail/smtp/SMTPTransport$DigestMD5Authenticator.class */
    ...
    /* loaded from: kore-javamail-1.0.2.jar:com/sun/mail/smtp/SMTPTransport$NtlmAuthenticator.class */
    ...
    public boolean sasllogin(String[] allowed, String realm, String authzid, String u, String p) throws MessagingException {
        ...
        try {
            ...
            boolean authenticate = this.saslAuthenticator.authenticate(mechs, realm, authzid, u, p);
            resumeTracing();
            return authenticate;
        } catch (Throwable th) {
            resumeTracing();
    ...
    protected void rcptTo() throws MessagingException {
        ...
        if (sendFailed) {
            ...
            for (int j = 0; j < valid.size(); j++) {
                int i3 = i2;
                i2++;
                this.validUnsentAddr[i3] = (Address) valid.elementAt(j);
            }
            for (int j2 = 0; j2 < validUnsent.size(); j2++) {
                int i4 = i2;
                i2++;
                this.validUnsentAddr[i4] = (Address) validUnsent.elementAt(j2);
            }
        ...
        if (this.logger.isLoggable(Level.FINE)) {
            ...
            if (this.validUnsentAddr != null && this.validUnsentAddr.length > 0) {
                ...
                for (int j3 = 0; j3 < this.validUnsentAddr.length; j3++) {
                    this.logger.fine("  " + this.validUnsentAddr[j3]);
                }
            }
    ...
    private void openServer(String host, int port) throws MessagingException {
        ...
        try {
            ...
            if (r != 220) {
                ...
                this.lineInputStream = null;
                if (this.logger.isLoggable(Level.FINE)) {
                    this.logger.fine("could not connect to host \"" + host + "\", port: " + port2 + ", response: " + r + "\n");
                }
                throw new MessagingException("Could not connect to SMTP host: " + host + ", port: " + port2 + ", response: " + r);
            }
            if (this.logger.isLoggable(Level.FINE)) {
                this.logger.fine("connected to host \"" + host + "\", port: " + port2 + "\n");
            }
    ...
    private void openServer() throws MessagingException {
        ...
        try {
            ...
            throw new MessagingException("Could not start protocol to SMTP host: " + this.host + ", port: -1", ioe);
        }
    }
    ...
    protected int readServerResponse() throws MessagingException {
        ...
        if (returnCode == -1) {
            this.logger.log(Level.FINE, "bad server response: {0}", serverResponse2);
        }
