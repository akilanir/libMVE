...
/* loaded from: TTS_library_stub.jar:com/google/tts/ConfigurationManager.class */
public class ConfigurationManager extends Activity {
    ...
    /* loaded from: TTS_library_stub.jar:com/google/tts/ConfigurationManager$dataDownloader.class */
    ...
    public static boolean allFilesExist() {
        ...
        for (String str : datafiles) {
            File tempFile = new File(String.valueOf("/sdcard/espeak-data/") + str);
            if (!tempFile.exists()) {
                for (String str2 : datafiles) {
                    File delFile = new File(String.valueOf("/sdcard/espeak-data/") + str2);
                    delFile.delete();
                }
                return false;
            }
    ...
    public static void downloadEspeakData() {
        try {
            ...
            File espeakDataDir = new File("/sdcard/espeak-data/");
            espeakDataDir.mkdir();
            String fileUrl = new URL(new URL("http://eyes-free.googlecode.com/svn/trunk/thirdparty/espeak-data.zip"), "http://eyes-free.googlecode.com/svn/trunk/thirdparty/espeak-data.zip").toString();
            ...
            while (zippedFiles.hasMoreElements()) {
                ...
                if (entry.isDirectory()) {
                    ...
                    String outputPath = new File("/sdcard/espeak-data/" + name).getCanonicalPath();
                    String name2 = outputPath.substring(outputPath.lastIndexOf("/") + 1);
                    String outputPath2 = outputPath.substring(0, outputPath.lastIndexOf("/"));
                    File outputDir = new File(outputPath2);
                    outputDir.mkdirs();
                    File outputFile = new File(outputPath2, name2);
                    outputFile.createNewFile();
                    FileOutputStream out2 = new FileOutputStream(outputFile);
                    byte[] buf2 = new byte[16384];
                    while (true) {
                        int numread2 = is.read(buf2);
                        if (numread2 <= 0) {
                            break;
                        } else {
