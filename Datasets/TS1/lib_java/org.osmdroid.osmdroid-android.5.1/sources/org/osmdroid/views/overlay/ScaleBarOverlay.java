...
/* loaded from: org.osmdroid.osmdroid-android.5.1.jar:org/osmdroid/views/overlay/ScaleBarOverlay.class */
public class ScaleBarOverlay extends Overlay implements GeoConstants {
    ...
    /* loaded from: org.osmdroid.osmdroid-android.5.1.jar:org/osmdroid/views/overlay/ScaleBarOverlay$UnitsOfMeasure.class */
    ...
    public ScaleBarOverlay(MapView mapView, ResourceProxy pResourceProxy) {
    ...
    private void drawLatitudeText(Canvas canvas, Projection projection) {
        ...
        if (this.alignRight) {
            x += this.screenWidth - xBarLengthPixels;
        }
        float y = this.alignBottom ? this.screenHeight - (xTextSpacing * 2) : sTextBoundsRect.height() + xTextSpacing;
    ...
    private void drawLongitudeText(Canvas canvas, Projection projection) {
        ...
        float x = this.alignRight ? this.screenWidth - (yTextSpacing * 2) : sTextBoundsRect.height() + yTextSpacing;
        float y = (yBarLengthPixels / 2) + (sTextBoundsRect.width() / 2);
        if (this.alignBottom) {
            y += this.screenHeight - yBarLengthPixels;
        }
        canvas.save();
    ...
    protected void rebuildBarPath(Projection projection) {
        ...
        if (this.latitudeBar) {
            this.barPath.moveTo(barToX, barOriginY + xTextHeight + (xTextSpacing * 2));
            this.barPath.lineTo(barToX, barOriginY);
            this.barPath.lineTo(barOriginX, barOriginY);
            if (!this.longitudeBar) {
                this.barPath.lineTo(barOriginX, barOriginY + xTextHeight + (xTextSpacing * 2));
            }
            this.latitudeBarRect.set(barOriginX, barOriginY, barToX, barOriginY + xTextHeight + (xTextSpacing * 2));
        }
        if (this.longitudeBar) {
            if (!this.latitudeBar) {
                this.barPath.moveTo(barOriginX + yTextHeight + (yTextSpacing * 2), barOriginY);
                this.barPath.lineTo(barOriginX, barOriginY);
            }
            this.barPath.lineTo(barOriginX, barToY);
            this.barPath.lineTo(barOriginX + yTextHeight + (yTextSpacing * 2), barToY);
