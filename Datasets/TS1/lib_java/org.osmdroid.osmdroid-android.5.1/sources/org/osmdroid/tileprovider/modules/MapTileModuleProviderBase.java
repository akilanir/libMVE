...
/* loaded from: org.osmdroid.osmdroid-android.5.1.jar:org/osmdroid/tileprovider/modules/MapTileModuleProviderBase.class */
public abstract class MapTileModuleProviderBase {
    ...
    public void loadMapTileAsync(MapTileRequestState pState) {
        ...
        synchronized (this.mQueueLockObject) {
            if (OpenStreetMapTileProviderConstants.DEBUG_TILE_PROVIDERS) {
                Log.d(IMapView.LOGTAG, "MapTileModuleProviderBase.loadMaptileAsync() on provider: " + getName() + " for tile: " + pState.getMapTile());
    ...
    /* loaded from: org.osmdroid.osmdroid-android.5.1.jar:org/osmdroid/tileprovider/modules/MapTileModuleProviderBase$TileLoader.class */
    protected abstract class TileLoader implements Runnable {
        ...
        protected MapTileRequestState nextTile() {
            ...
            synchronized (MapTileModuleProviderBase.this.mQueueLockObject) {
                ...
                for (MapTile tile : MapTileModuleProviderBase.this.mPending.keySet()) {
                    if (!MapTileModuleProviderBase.this.mWorking.containsKey(tile)) {
                        if (OpenStreetMapTileProviderConstants.DEBUG_TILE_PROVIDERS) {
                            Log.d(IMapView.LOGTAG, "TileLoader.nextTile() on provider: " + MapTileModuleProviderBase.this.getName() + " found tile in working queue: " + tile);
                        }
                        result = tile;
                    }
                }
                if (result != null) {
                    if (OpenStreetMapTileProviderConstants.DEBUG_TILE_PROVIDERS) {
                        Log.d(IMapView.LOGTAG, "TileLoader.nextTile() on provider: " + MapTileModuleProviderBase.this.getName() + " adding tile to working queue: " + result);
                    }
        ...
        protected void tileLoaded(MapTileRequestState pState, Drawable pDrawable) {
            if (OpenStreetMapTileProviderConstants.DEBUG_TILE_PROVIDERS) {
                Log.d(IMapView.LOGTAG, "TileLoader.tileLoaded() on provider: " + MapTileModuleProviderBase.this.getName() + " with tile: " + pState.getMapTile());
            }
        ...
        protected void tileLoadedExpired(MapTileRequestState pState, Drawable pDrawable) {
            if (OpenStreetMapTileProviderConstants.DEBUG_TILE_PROVIDERS) {
                Log.d(IMapView.LOGTAG, "TileLoader.tileLoadedExpired() on provider: " + MapTileModuleProviderBase.this.getName() + " with tile: " + pState.getMapTile());
            }
            MapTileModuleProviderBase.this.removeTileFromQueues(pState.getMapTile());
        ...
        protected void tileLoadedFailed(MapTileRequestState pState) {
            if (OpenStreetMapTileProviderConstants.DEBUG_TILE_PROVIDERS) {
                Log.d(IMapView.LOGTAG, "TileLoader.tileLoadedFailed() on provider: " + MapTileModuleProviderBase.this.getName() + " with tile: " + pState.getMapTile());
            }
            MapTileModuleProviderBase.this.removeTileFromQueues(pState.getMapTile());
    ...
    /* loaded from: org.osmdroid.osmdroid-android.5.1.jar:org/osmdroid/tileprovider/modules/MapTileModuleProviderBase$CantContinueException.class */
