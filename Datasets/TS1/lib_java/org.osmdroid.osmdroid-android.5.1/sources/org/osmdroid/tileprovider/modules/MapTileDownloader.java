...
/* loaded from: org.osmdroid.osmdroid-android.5.1.jar:org/osmdroid/tileprovider/modules/MapTileDownloader.class */
public class MapTileDownloader extends MapTileModuleProviderBase {
    public static long MAX_CACHED_TILE_AGE = 604800000;
    ...
    public MapTileDownloader(ITileSource pTileSource, IFilesystemCache pFilesystemCache, INetworkAvailablityCheck pNetworkAvailablityCheck) {
        this(pTileSource, pFilesystemCache, pNetworkAvailablityCheck, OpenStreetMapTileProviderConstants.getNumberOfTileDownloadThreads(), 40);
    }
    ...
    /* loaded from: org.osmdroid.osmdroid-android.5.1.jar:org/osmdroid/tileprovider/modules/MapTileDownloader$TileLoader.class */
        ...
        public Drawable loadTile(MapTileRequestState aState) throws MapTileModuleProviderBase.CantContinueException {
            ...
            try {
                try {
                    try {
                        try {
                            try {
                                ...
                                Date dateExpires = new Date(System.currentTimeMillis() + MapTileDownloader.MAX_CACHED_TILE_AGE);
                                String expires = c2.getHeaderField(OpenStreetMapTileProviderConstants.HTTP_EXPIRES_HEADER);
                                if (expires != null && expires.length() > 0) {
                                    try {
                                        dateExpires = OpenStreetMapTileProviderConstants.HTTP_HEADER_SDF.parse(expires);
                                    } catch (Exception ex) {
                                        if (MapTileDownloader.DEBUG) {
                            ...
                            Log.w(IMapView.LOGTAG, "UnknownHostException downloading MapTile: " + tile + " : " + e6);
                    ...
                    Log.w(IMapView.LOGTAG, "LowMemoryException downloading MapTile: " + tile + " : " + e9);
                    throw new MapTileModuleProviderBase.CantContinueException(e9);
                }
            } catch (FileNotFoundException e10) {
                Log.w(IMapView.LOGTAG, "Tile not found: " + tile + " : " + e10);
                StreamUtils.closeStream(null);
                StreamUtils.closeStream(null);
                try {
                    c.disconnect();
                ...
                Log.w(IMapView.LOGTAG, "IOException downloading MapTile: " + tile + " : " + e12);
                StreamUtils.closeStream(null);
                StreamUtils.closeStream(null);
                try {
                    c.disconnect();
