...
/* loaded from: MPAndroidChart-v2.2.5.jar:com/github/mikephil/charting/renderer/BubbleChartRenderer.class */
public class BubbleChartRenderer extends DataRenderer {
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (Highlight high : indices) {
            ...
            int maxDataSetIndex = high.getDataSetIndex() == -1 ? bubbleData.getDataSetCount() : high.getDataSetIndex() + 1;
            if (maxDataSetIndex - minDataSetIndex >= 1) {
                for (int dataSetIndex = minDataSetIndex; dataSetIndex < maxDataSetIndex; dataSetIndex++) {
                    IBubbleDataSet dataSet = (IBubbleDataSet) bubbleData.getDataSetByIndex(dataSetIndex);
                    if (dataSet != null && dataSet.isHighlightEnabled() && (entry = (BubbleEntry) bubbleData.getEntryForHighlight(high)) != null && entry.getXIndex() == high.getXIndex()) {
                        BubbleEntry entryFrom = (BubbleEntry) dataSet.getEntryForXIndex(this.mMinX);
                        BubbleEntry entryTo = (BubbleEntry) dataSet.getEntryForXIndex(this.mMaxX);
                        int minx = dataSet.getEntryIndex(entryFrom);
                        int maxx = Math.min(dataSet.getEntryIndex(entryTo) + 1, dataSet.getEntryCount());
