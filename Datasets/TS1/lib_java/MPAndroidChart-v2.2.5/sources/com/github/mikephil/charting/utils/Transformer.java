...
/* loaded from: MPAndroidChart-v2.2.5.jar:com/github/mikephil/charting/utils/Transformer.class */
public class Transformer {
    ...
    public float[] generateTransformedValuesScatter(IScatterDataSet data, float phaseY) {
        ...
        for (int j = 0; j < valuePoints.length; j += 2) {
            ?? entryForIndex = data.getEntryForIndex(j / 2);
            if (entryForIndex != 0) {
                valuePoints[j] = entryForIndex.getXIndex();
                valuePoints[j + 1] = entryForIndex.getVal() * phaseY;
            }
        }
    ...
    public float[] generateTransformedValuesBubble(IBubbleDataSet data, float phaseX, float phaseY, int from, int to) {
        ...
        for (int j = 0; j < count; j += 2) {
            ...
            if (entryForIndex != 0) {
                valuePoints[j] = ((entryForIndex.getXIndex() - from) * phaseX) + from;
                valuePoints[j + 1] = entryForIndex.getVal() * phaseY;
            }
        }
        getValueToPixelMatrix().mapPoints(valuePoints);
    ...
    public float[] generateTransformedValuesBarChart(IBarDataSet data, int dataSetIndex, BarData bd, float phaseY) {
        ...
        for (int j = 0; j < valuePoints.length; j += 2) {
            ?? entryForIndex = data.getEntryForIndex(j / 2);
            int i = entryForIndex.getXIndex();
            float x = entryForIndex.getXIndex() + (i * (setCount - 1)) + dataSetIndex + (space * i) + (space / 2.0f);
            float y = entryForIndex.getVal();
            valuePoints[j] = x;
    ...
    public float[] generateTransformedValuesHorizontalBarChart(IBarDataSet data, int dataSet, BarData bd, float phaseY) {
        ...
        for (int j = 0; j < valuePoints.length; j += 2) {
            ?? entryForIndex = data.getEntryForIndex(j / 2);
            int i = entryForIndex.getXIndex();
            float x = i + (i * (setCount - 1)) + dataSet + (space * i) + (space / 2.0f);
            float y = entryForIndex.getVal();
