...
/* loaded from: jackson-databind-2.6.1.jar:com/fasterxml/jackson/databind/deser/BasicDeserializerFactory.class */
public abstract class BasicDeserializerFactory extends DeserializerFactory implements Serializable {
    ...
    */
    ...
    protected SettableBeanProperty constructCreatorProperty(DeserializationContext ctxt, BeanDescription beanDesc, PropertyName name, int index, AnnotatedParameter param, Object injectableValueId) throws JsonMappingException {
        ...
        SettableBeanProperty prop = new CreatorProperty(name, type2, property.getWrapperName(), typeDeser, beanDesc.getClassAnnotations(), param, index, injectableValueId, metadata);
        if (deser != null) {
    ...
    public JsonDeserializer<?> createArrayDeserializer(DeserializationContext ctxt, ArrayType type, BeanDescription beanDesc) throws JsonMappingException {
        DeserializationConfig config = ctxt.getConfig();
        JavaType elemType = type.mo4getContentType();
    ...
    */
    ...
    public TypeDeserializer findPropertyContentTypeDeserializer(DeserializationConfig config, JavaType containerType, AnnotatedMember propertyEntity) throws JsonMappingException {
        ...
        JavaType contentType = containerType.mo4getContentType();
        if (b == null) {
    ...
    public JsonDeserializer<?> findDefaultDeserializer(DeserializationContext ctxt, JavaType type, BeanDescription beanDesc) throws JsonMappingException {
        ...
        if (type.isReferenceType()) {
            JavaType referencedType = type.mo3getReferencedType();
            if (AtomicReference.class.isAssignableFrom(rawType)) {
                ...
                return new AtomicReferenceDeserializer(referencedType, vts, findDeserializerFromAnnotation(ctxt, refdDesc.getClassInfo()));
            }
        }
        ...
        if (rawType == CLASS_MAP_ENTRY) {
            JavaType kt = type.mo2containedType(0);
            if (kt == null) {
                kt = TypeFactory.unknownType();
            }
            JavaType vt = type.mo2containedType(1);
            if (vt == null) {
                vt = TypeFactory.unknownType();
            }
            TypeDeserializer vts2 = (TypeDeserializer) vt.getTypeHandler();
            if (vts2 == null) {
                vts2 = findTypeDeserializer(ctxt.getConfig(), vt);
            }
            JsonDeserializer<Object> valueDeser = (JsonDeserializer) vt.getValueHandler();
            KeyDeserializer keyDes = (KeyDeserializer) kt.getValueHandler();
            return new MapEntryDeserializer(type, keyDes, valueDeser, vts2);
        }
    ...
    /* JADX WARN: Multi-variable type inference failed */
    /* JADX WARN: Type inference failed for: r0v52, types: [com.fasterxml.jackson.databind.JavaType] */
    protected <T extends JavaType> T modifyTypeByAnnotation(DeserializationContext deserializationContext, Annotated annotated, T t) throws JsonMappingException {
        ...
        if (findDeserializationType != null) {
            try {
                t = (T) deserializationContext.getTypeFactory().constructSpecializedType(t, findDeserializationType);
        ...
        T t2 = t;
        if (isContainerType) {
            Class<?> findDeserializationKeyType = annotationIntrospector.findDeserializationKeyType(annotated, t.mo5getKeyType());
            ...
            JavaType mo5getKeyType = t.mo5getKeyType();
            if (mo5getKeyType != null && mo5getKeyType.getValueHandler() == null && (keyDeserializerInstance = deserializationContext.keyDeserializerInstance(annotated, annotationIntrospector.findKeyDeserializer(annotated))) != null) {
                t = ((MapLikeType) t).withKeyValueHandler(keyDeserializerInstance);
                t.mo5getKeyType();
            }
            Class<?> findDeserializationContentType = annotationIntrospector.findDeserializationContentType(annotated, t.mo4getContentType());
            ...
            Object valueHandler = t.mo4getContentType().getValueHandler();
            t2 = t;
