...
/* loaded from: okhttp-2.2.0.jar:com/squareup/okhttp/internal/spdy/Http20Draft16.class */
public final class Http20Draft16 implements Variant {
    ...
    /* loaded from: okhttp-2.2.0.jar:com/squareup/okhttp/internal/spdy/Http20Draft16$Reader.class */
    static final class Reader implements FrameReader {
        ...
        Reader(BufferedSource source, int headerTableSize, boolean client) {
            ...
            this.hpackReader = new HpackDraft10.Reader(headerTableSize, this.continuation);
        }
        ...
        public void readConnectionPreface() throws IOException {
            ...
            ByteString connectionPreface = this.source.readByteString(Http20Draft16.CONNECTION_PREFACE.size());
            if (Http20Draft16.logger.isLoggable(Level.FINE)) {
                Http20Draft16.logger.fine(String.format("<< CONNECTION %s", connectionPreface.hex()));
            }
            if (!Http20Draft16.CONNECTION_PREFACE.equals(connectionPreface)) {
                throw Http20Draft16.ioException("Expected a connection header but was %s", connectionPreface.utf8());
            }
        }
        ...
        public boolean nextFrame(FrameReader.Handler handler) throws IOException {
            try {
                ...
                int length = Http20Draft16.readMedium(this.source);
                if (length < 0 || length > Http20Draft16.INITIAL_MAX_FRAME_SIZE) {
                    throw Http20Draft16.ioException("FRAME_SIZE_ERROR: %s", Integer.valueOf(length));
                }
                ...
                if (Http20Draft16.logger.isLoggable(Level.FINE)) {
                    Http20Draft16.logger.fine(FrameLogger.formatHeader(true, streamId, length, type, flags));
                }
                switch (type) {
                    case 0:
                    ...
                    case 1:
                        ...
                        return true;
                    case Http20Draft16.TYPE_PRIORITY /* 2 */:
                        readPriority(handler, length, flags, streamId);
                        return true;
                    case Http20Draft16.TYPE_RST_STREAM /* 3 */:
                        readRstStream(handler, length, flags, streamId);
                        return true;
                    case 4:
                        readSettings(handler, length, flags, streamId);
                        return true;
                    case Http20Draft16.TYPE_PUSH_PROMISE /* 5 */:
                        readPushPromise(handler, length, flags, streamId);
                        return true;
                    case Http20Draft16.TYPE_PING /* 6 */:
                        readPing(handler, length, flags, streamId);
                        return true;
                    case Http20Draft16.TYPE_GOAWAY /* 7 */:
        ...
        private void readHeaders(FrameReader.Handler handler, int length, byte flags, int streamId) throws IOException {
            ...
            List<Header> headerBlock = readHeaderBlock(Http20Draft16.lengthWithoutPadding(length, flags, padding), padding, flags, streamId);
        ...
        private void readData(FrameReader.Handler handler, int length, byte flags, int streamId) throws IOException {
            ...
            short padding = (flags & 8) != 0 ? (short) (this.source.readByte() & 255) : (short) 0;
            handler.data(inFinished, streamId, this.source, Http20Draft16.lengthWithoutPadding(length, flags, padding));
        ...
        private void readPriority(FrameReader.Handler handler, int length, byte flags, int streamId) throws IOException {
            if (length != Http20Draft16.TYPE_PUSH_PROMISE) {
                throw Http20Draft16.ioException("TYPE_PRIORITY length: %d != 5", Integer.valueOf(length));
            }
            if (streamId == 0) {
        ...
        private void readRstStream(FrameReader.Handler handler, int length, byte flags, int streamId) throws IOException {
            if (length != 4) {
                throw Http20Draft16.ioException("TYPE_RST_STREAM length: %d != 4", Integer.valueOf(length));
            }
            if (streamId == 0) {
            ...
            if (errorCode == null) {
                throw Http20Draft16.ioException("TYPE_RST_STREAM unexpected error code: %d", Integer.valueOf(errorCodeInt));
            }
        ...
        private void readSettings(FrameReader.Handler handler, int length, byte flags, int streamId) throws IOException {
            ...
            if (length % Http20Draft16.TYPE_PING != 0) {
                throw Http20Draft16.ioException("TYPE_SETTINGS length %% 6 != 0: %s", Integer.valueOf(length));
            }
            Settings settings = new Settings();
            for (int i = 0; i < length; i += Http20Draft16.TYPE_PING) {
                ...
                switch (id) {
                    ...
                    case Http20Draft16.TYPE_RST_STREAM /* 3 */:
                        ...
                        break;
                    case 4:
                        id = Http20Draft16.TYPE_GOAWAY;
                        if (value < 0) {
                            throw Http20Draft16.ioException("PROTOCOL_ERROR SETTINGS_INITIAL_WINDOW_SIZE > 2^31 - 1", new Object[0]);
                        }
                        break;
                    case Http20Draft16.TYPE_PUSH_PROMISE /* 5 */:
                        if (value < Http20Draft16.INITIAL_MAX_FRAME_SIZE || value > 16777215) {
                            throw Http20Draft16.ioException("PROTOCOL_ERROR SETTINGS_MAX_FRAME_SIZE: %s", Integer.valueOf(value));
                        }
                        break;
                        break;
                    default:
                        throw Http20Draft16.ioException("PROTOCOL_ERROR invalid settings id: %s", Short.valueOf(id));
                }
        ...
        private void readPushPromise(FrameReader.Handler handler, int length, byte flags, int streamId) throws IOException {
            ...
            List<Header> headerBlock = readHeaderBlock(Http20Draft16.lengthWithoutPadding(length - 4, flags, padding), padding, flags, streamId);
            handler.pushPromise(streamId, promisedStreamId, headerBlock);
        }

        private void readPing(FrameReader.Handler handler, int length, byte flags, int streamId) throws IOException {
            if (length != 8) {
                throw Http20Draft16.ioException("TYPE_PING length != 8: %s", Integer.valueOf(length));
            }
            if (streamId != 0) {
        ...
        private void readGoAway(FrameReader.Handler handler, int length, byte flags, int streamId) throws IOException {
            if (length < 8) {
                throw Http20Draft16.ioException("TYPE_GOAWAY length < 8: %s", Integer.valueOf(length));
            }
            if (streamId != 0) {
            ...
            ErrorCode errorCode = ErrorCode.fromHttp2(errorCodeInt);
            if (errorCode == null) {
                throw Http20Draft16.ioException("TYPE_GOAWAY unexpected error code: %d", Integer.valueOf(errorCodeInt));
            }
        ...
        private void readWindowUpdate(FrameReader.Handler handler, int length, byte flags, int streamId) throws IOException {
            if (length != 4) {
                throw Http20Draft16.ioException("TYPE_WINDOW_UPDATE length !=4: %s", Integer.valueOf(length));
            }
            long increment = this.source.readInt() & 2147483647L;
            if (increment == 0) {
                throw Http20Draft16.ioException("windowSizeIncrement was 0", Long.valueOf(increment));
            }
    ...
    /* loaded from: okhttp-2.2.0.jar:com/squareup/okhttp/internal/spdy/Http20Draft16$Writer.class */
    static final class Writer implements FrameWriter {
        private final BufferedSink sink;
        private final boolean client;
        private final Buffer hpackBuffer = new Buffer();
        private final HpackDraft10.Writer hpackWriter = new HpackDraft10.Writer(this.hpackBuffer);
        private int maxFrameSize = Http20Draft16.INITIAL_MAX_FRAME_SIZE;
        ...
        public synchronized void connectionPreface() throws IOException {
            ...
            if (this.client) {
                if (Http20Draft16.logger.isLoggable(Level.FINE)) {
                    Http20Draft16.logger.fine(String.format(">> CONNECTION %s", Http20Draft16.CONNECTION_PREFACE.hex()));
                }
        ...
        public synchronized void settings(Settings settings) throws IOException {
            ...
            int length = settings.size() * Http20Draft16.TYPE_PING;
            frameHeader(0, length, (byte) 4, (byte) 0);
            for (int i = 0; i < 10; i++) {
                if (settings.isSet(i)) {
                    int id = i;
                    if (id == 4) {
                        id = Http20Draft16.TYPE_RST_STREAM;
                    } else if (id == Http20Draft16.TYPE_GOAWAY) {
                        id = 4;
                    }
        ...
        public synchronized void windowUpdate(int streamId, long windowSizeIncrement) throws IOException {
            ...
            if (windowSizeIncrement == 0 || windowSizeIncrement > 2147483647L) {
                throw Http20Draft16.illegalArgument("windowSizeIncrement == 0 || windowSizeIncrement > 0x7fffffffL: %s", Long.valueOf(windowSizeIncrement));
            }
        ...
        void frameHeader(int streamId, int length, byte type, byte flags) throws IOException {
            if (Http20Draft16.logger.isLoggable(Level.FINE)) {
                Http20Draft16.logger.fine(FrameLogger.formatHeader(false, streamId, length, type, flags));
            }
            if (length > this.maxFrameSize) {
                throw Http20Draft16.illegalArgument("FRAME_SIZE_ERROR length > %d: %d", Integer.valueOf(this.maxFrameSize), Integer.valueOf(length));
            }
            if ((streamId & Integer.MIN_VALUE) != 0) {
                throw Http20Draft16.illegalArgument("reserved bit set: %s", Integer.valueOf(streamId));
            }
            Http20Draft16.writeMedium(this.sink, length);
    ...
    /* loaded from: okhttp-2.2.0.jar:com/squareup/okhttp/internal/spdy/Http20Draft16$ContinuationSource.class */
    static final class ContinuationSource implements Source {
        ...
        private void readContinuationHeader() throws IOException {
            ...
            int readMedium = Http20Draft16.readMedium(this.source);
            this.left = readMedium;
            this.length = readMedium;
            byte type = (byte) (this.source.readByte() & 255);
            this.flags = (byte) (this.source.readByte() & 255);
            if (Http20Draft16.logger.isLoggable(Level.FINE)) {
                Http20Draft16.logger.fine(FrameLogger.formatHeader(true, this.streamId, this.length, type, this.flags));
            }
            this.streamId = this.source.readInt() & Integer.MAX_VALUE;
            if (type != Http20Draft16.TYPE_CONTINUATION) {
                throw Http20Draft16.ioException("%s != TYPE_CONTINUATION", Byte.valueOf(type));
            }
    ...
    /* loaded from: okhttp-2.2.0.jar:com/squareup/okhttp/internal/spdy/Http20Draft16$FrameLogger.class */
    static final class FrameLogger {
        ...
        static String formatHeader(boolean inbound, int streamId, int length, byte type, byte flags) {
            ...
            Object[] objArr = new Object[Http20Draft16.TYPE_PUSH_PROMISE];
            objArr[0] = inbound ? "<<" : ">>";
            objArr[1] = Integer.valueOf(streamId);
            objArr[Http20Draft16.TYPE_PRIORITY] = Integer.valueOf(length);
            objArr[Http20Draft16.TYPE_RST_STREAM] = formattedType;
        ...
        static String formatFlags(byte type, byte flags) {
            ...
            switch (type) {
                ...
                default:
                    ...
                    if (type == Http20Draft16.TYPE_PUSH_PROMISE && (flags & 4) != 0) {
