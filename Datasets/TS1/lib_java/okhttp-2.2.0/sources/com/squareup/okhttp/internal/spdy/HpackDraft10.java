...
/* loaded from: okhttp-2.2.0.jar:com/squareup/okhttp/internal/spdy/HpackDraft10.class */
final class HpackDraft10 {
    ...
    /* loaded from: okhttp-2.2.0.jar:com/squareup/okhttp/internal/spdy/HpackDraft10$Reader.class */
    static final class Reader {
        ...
        void readHeaders() throws IOException {
            while (!this.source.exhausted()) {
                ...
                if ((b & 128) == 128) {
                    int index = readInt(b, HpackDraft10.PREFIX_7_BITS);
                    readIndexedHeader(index - 1);
                } else if (b == 64) {
                    readLiteralHeaderWithIncrementalIndexingNewName();
                } else if ((b & 64) == 64) {
                    int index2 = readInt(b, HpackDraft10.PREFIX_6_BITS);
                    readLiteralHeaderWithIncrementalIndexingIndexedName(index2 - 1);
                } else if ((b & 32) == 32) {
                    this.maxDynamicTableByteCount = readInt(b, HpackDraft10.PREFIX_5_BITS);
                    ...
                    int index3 = readInt(b, HpackDraft10.PREFIX_4_BITS);
        ...
        private void readIndexedHeader(int index) throws IOException {
            if (isStaticHeader(index)) {
                Header staticEntry = HpackDraft10.STATIC_HEADER_TABLE[index];
                this.headerList.add(staticEntry);
                return;
            }
            int dynamicTableIndex = dynamicTableIndex(index - HpackDraft10.STATIC_HEADER_TABLE.length);
        ...
        private void readLiteralHeaderWithoutIndexingNewName() throws IOException {
            ByteString name = HpackDraft10.checkLowercase(readByteString());
            ByteString value = readByteString();
            this.headerList.add(new Header(name, value));
        }
        ...
        private ByteString getName(int index) {
            return isStaticHeader(index) ? HpackDraft10.STATIC_HEADER_TABLE[index].name : this.dynamicTable[dynamicTableIndex(index - HpackDraft10.STATIC_HEADER_TABLE.length)].name;
        }

        private boolean isStaticHeader(int index) {
            return index >= 0 && index <= HpackDraft10.STATIC_HEADER_TABLE.length - 1;
        }
        ...
        int readInt(int firstByte, int prefixMask) throws IOException {
            ...
            while (true) {
                int b = readByte();
                if ((b & 128) != 0) {
                    result += (b & HpackDraft10.PREFIX_7_BITS) << shift;
                    shift += 7;
                } else {
        ...
        ByteString readByteString() throws IOException {
            ...
            int length = readInt(firstByte, HpackDraft10.PREFIX_7_BITS);
            if (huffmanDecode) {
    ...
    /* loaded from: okhttp-2.2.0.jar:com/squareup/okhttp/internal/spdy/HpackDraft10$Writer.class */
    static final class Writer {
        ...
        void writeHeaders(List<Header> headerBlock) throws IOException {
            ...
            for (int i = 0; i < size; i++) {
                ...
                Integer staticIndex = (Integer) HpackDraft10.NAME_TO_FIRST_INDEX.get(name);
                if (staticIndex != null) {
                    writeInt(staticIndex.intValue() + 1, HpackDraft10.PREFIX_4_BITS, 0);
        ...
        void writeInt(int value, int prefixMask, int bits) throws IOException {
            ...
            while (true) {
                int value2 = i;
                if (value2 >= 128) {
                    int b = value2 & HpackDraft10.PREFIX_7_BITS;
        ...
        void writeByteString(ByteString data) throws IOException {
            writeInt(data.size(), HpackDraft10.PREFIX_7_BITS, 0);
            this.out.write(data);
        }
    }
