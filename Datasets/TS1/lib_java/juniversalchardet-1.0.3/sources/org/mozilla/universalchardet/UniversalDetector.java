...
/* loaded from: juniversalchardet-1.0.3.jar:org/mozilla/universalchardet/UniversalDetector.class */
public class UniversalDetector {
    ...
    /* loaded from: juniversalchardet-1.0.3.jar:org/mozilla/universalchardet/UniversalDetector$InputState.class */
    ...
    public void handleData(byte[] buf, int offset, int length) {
        ...
        if (this.start) {
            this.start = false;
            if (length > 3) {
                int b1 = buf[offset] & PkgInt.UNIT_MASK_8BITS;
                int b2 = buf[offset + 1] & PkgInt.UNIT_MASK_8BITS;
                int b3 = buf[offset + 2] & PkgInt.UNIT_MASK_8BITS;
                int b4 = buf[offset + 3] & PkgInt.UNIT_MASK_8BITS;
                switch (b1) {
                    case 0:
                    ...
                    case 254:
                        ...
                        break;
                    case PkgInt.UNIT_MASK_8BITS /* 255 */:
                        if (b2 != 254 || b3 != 0 || b4 != 0) {
                            if (b2 == 254) {
        ...
        for (int i = offset; i < maxPos; i++) {
            int c = buf[i] & PkgInt.UNIT_MASK_8BITS;
            if ((c & SJISDistributionAnalysis.LOWBYTE_BEGIN_2) != 0 && c != 160) {
        ...
        if (this.inputState == InputState.ESC_ASCII) {
            ...
            CharsetProber.ProbingState st = this.escCharsetProber.handleData(buf, offset, length);
        ...
        if (this.inputState == InputState.HIGHBYTE) {
            for (int i2 = 0; i2 < this.probers.length; i2++) {
                CharsetProber.ProbingState st2 = this.probers[i2].handleData(buf, offset, length);
                if (st2 == CharsetProber.ProbingState.FOUND_IT) {
                    this.done = true;
    ...
    public static void main(String[] args) throws Exception {
        ...
        UniversalDetector detector = new UniversalDetector(new CharsetListener() { // from class: org.mozilla.universalchardet.UniversalDetector.1
