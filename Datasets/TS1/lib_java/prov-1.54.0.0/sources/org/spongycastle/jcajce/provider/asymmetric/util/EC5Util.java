...
/* loaded from: prov-1.54.0.0.jar:org/spongycastle/jcajce/provider/asymmetric/util/EC5Util.class */
public class EC5Util {
    ...
    public static ECParameterSpec convertToSpec(X962Parameters params, ECCurve curve) {
        ...
        if (params.isNamedCurve()) {
            ...
            X9ECParameters ecP = ECUtil.getNamedCurveByOid(oid);
            ecSpec = new ECNamedCurveSpec(ECUtil.getCurveName(oid), convertCurve(curve, ecP.getSeed()), new ECPoint(ecP.getG().getAffineXCoord().toBigInteger(), ecP.getG().getAffineYCoord().toBigInteger()), ecP.getN(), ecP.getH());
        } else if (params.isImplicitlyCA()) {
            ecSpec = null;
        } else {
            X9ECParameters ecP2 = X9ECParameters.getInstance(params.getParameters());
    ...
    public static org.spongycastle.jce.spec.ECParameterSpec convertSpec(ECParameterSpec ecSpec, boolean withCompression) {
        ...
        return new org.spongycastle.jce.spec.ECParameterSpec(curve, convertPoint(curve, ecSpec.getGenerator(), withCompression), ecSpec.getOrder(), BigInteger.valueOf(ecSpec.getCofactor()), ecSpec.getCurve().getSeed());
    }
