...
/* loaded from: prov-1.54.0.0.jar:org/spongycastle/jcajce/provider/asymmetric/x509/CertificateFactory.class */
public class CertificateFactory extends CertificateFactorySpi {
    ...
    private Certificate readDERCertificate(ASN1InputStream dIn) throws IOException, CertificateParsingException {
        ...
        if (seq.size() > 1 && (seq.getObjectAt(0) instanceof ASN1ObjectIdentifier) && seq.getObjectAt(0).equals(PKCSObjectIdentifiers.signedData)) {
            this.sData = SignedData.getInstance(ASN1Sequence.getInstance(seq.getObjectAt(1), true)).getCertificates();
    ...
    private CRL readDERCRL(ASN1InputStream aIn) throws IOException, CRLException {
        ASN1Sequence seq = aIn.readObject();
        if (seq.size() > 1 && (seq.getObjectAt(0) instanceof ASN1ObjectIdentifier) && seq.getObjectAt(0).equals(PKCSObjectIdentifiers.signedData)) {
            this.sCrlData = SignedData.getInstance(ASN1Sequence.getInstance(seq.getObjectAt(1), true)).getCRLs();
            return getCRL();
        }
        return createCRL(CertificateList.getInstance(seq));
    }
    ...
    /* loaded from: prov-1.54.0.0.jar:org/spongycastle/jcajce/provider/asymmetric/x509/CertificateFactory$ExCertificateException.class */
