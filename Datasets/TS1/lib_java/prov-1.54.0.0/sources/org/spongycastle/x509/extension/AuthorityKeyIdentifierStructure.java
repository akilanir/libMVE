...
/* loaded from: prov-1.54.0.0.jar:org/spongycastle/x509/extension/AuthorityKeyIdentifierStructure.class */
public class AuthorityKeyIdentifierStructure extends AuthorityKeyIdentifier {
    public AuthorityKeyIdentifierStructure(byte[] encodedValue) throws IOException {
        super(X509ExtensionUtil.fromExtensionValue(encodedValue));
    }

    public AuthorityKeyIdentifierStructure(X509Extension extension) {
        super(extension.getParsedValue());
    }
    ...
    private static ASN1Sequence fromCertificate(X509Certificate certificate) throws CertificateParsingException {
        try {
            if (certificate.getVersion() != 3) {
                ...
                return new AuthorityKeyIdentifier(info, new GeneralNames(genName), certificate.getSerialNumber()).toASN1Primitive();
            }
            ...
            if (ext != null) {
                ASN1OctetString str = X509ExtensionUtil.fromExtensionValue(ext);
                return new AuthorityKeyIdentifier(str.getOctets(), new GeneralNames(genName2), certificate.getSerialNumber()).toASN1Primitive();
            }
            SubjectPublicKeyInfo info2 = SubjectPublicKeyInfo.getInstance(certificate.getPublicKey().getEncoded());
            return new AuthorityKeyIdentifier(info2, new GeneralNames(genName2), certificate.getSerialNumber()).toASN1Primitive();
        } catch (Exception e) {
    ...
    private static ASN1Sequence fromKey(PublicKey pubKey) throws InvalidKeyException {
        try {
            ...
            return new AuthorityKeyIdentifier(info).toASN1Primitive();
        } catch (Exception e) {
