...
/* loaded from: commons-digester-1.8.jar:org/apache/commons/digester/SetNextRule.class */
public class SetNextRule extends Rule {
    ...
    public void end() throws Exception {
        ...
        if (this.digester.log.isDebugEnabled()) {
            if (parent == null) {
                this.digester.log.debug(new StringBuffer().append("[SetNextRule]{").append(this.digester.match).append("} Call [NULL PARENT].").append(this.methodName).append("(").append(child).append(")").toString());
            } else {
                this.digester.log.debug(new StringBuffer().append("[SetNextRule]{").append(this.digester.match).append("} Call ").append(parent.getClass().getName()).append(".").append(this.methodName).append("(").append(child).append(")").toString());
            }
        }
