...
/* loaded from: jmdns-3.4.1.jar:javax/jmdns/impl/DNSOutgoing.class */
public final class DNSOutgoing extends DNSMessage {
    ...
    /* loaded from: jmdns-3.4.1.jar:javax/jmdns/impl/DNSOutgoing$MessageOutputStream.class */
    public static class MessageOutputStream extends ByteArrayOutputStream {
        ...
        void writeUTF(String str, int off, int len) {
            ...
            for (int i2 = 0; i2 < len; i2++) {
                ...
                if (ch2 >= 1 && ch2 <= 127) {
                    ...
                    writeByte(224 | ((ch2 >> DNSOutgoing.HEADER_SIZE) & 15));
                    writeByte(128 | ((ch2 >> 6) & 63));
                    writeByte(128 | ((ch2 >> 0) & 63));
                } else {
                    writeByte(192 | ((ch2 >> 6) & 31));
                    writeByte(128 | ((ch2 >> 0) & 63));
                }
            }
        }
        ...
        void writeRecord(DNSRecord rec, long now) {
            ...
            writeShort(rec.getRecordClass().indexValue() | ((rec.isUnique() && this._out.isMulticast()) ? 32768 : 0));
            writeInt(now == 0 ? rec.getTTL() : rec.getRemainingTTL(now));
            MessageOutputStream record = new MessageOutputStream(DNSConstants.FLAGS_TC, this._out, this._offset + size() + 2);
            rec.write(record);
    ...
    public int availableSpace() {
        return ((((this._maxUDPPayload - HEADER_SIZE) - this._questionsBytes.size()) - this._answersBytes.size()) - this._authoritativeAnswersBytes.size()) - this._additionalsAnswersBytes.size();
    }

    public void addQuestion(DNSQuestion rec) throws IOException {
        MessageOutputStream record = new MessageOutputStream(DNSConstants.FLAGS_TC, this);
