...
/* loaded from: com.mikepenz.iconics-core.2.5.11.jar:com/mikepenz/iconics/IconicsArrayBuilder.class */
    ...
    public IconicsDrawable[] build() {
        ...
        for (int i = 0; i < this.mIcons.size(); i++) {
            if (this.mIcons.get(i) instanceof IIcon) {
                iconicsDrawables[i] = this.mIconBase.m1clone().icon((IIcon) this.mIcons.get(i));
            } else if (this.mIcons.get(i) instanceof Character) {
                iconicsDrawables[i] = this.mIconBase.m1clone().icon((Character) this.mIcons.get(i));
            } else if (this.mIcons.get(i) instanceof String) {
                iconicsDrawables[i] = this.mIconBase.m1clone().iconText((String) this.mIcons.get(i));
            }
        }
