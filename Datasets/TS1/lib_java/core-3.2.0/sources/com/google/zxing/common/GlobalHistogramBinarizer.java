...
/* loaded from: core-3.2.0.jar:com/google/zxing/common/GlobalHistogramBinarizer.class */
public class GlobalHistogramBinarizer extends Binarizer {
    ...
    public BitMatrix getBlackMatrix() throws NotFoundException {
        ...
        initArrays(width);
        int[] localBuckets = this.buckets;
        for (int y = 1; y < LUMINANCE_BITS; y++) {
            int row = (height * y) / LUMINANCE_BITS;
            byte[] localLuminances = source.getRow(row, this.luminances);
            int right = (width * 4) / LUMINANCE_BITS;
            for (int x = width / LUMINANCE_BITS; x < right; x++) {
                int pixel = localLuminances[x] & 255;
                int i = pixel >> 3;
                localBuckets[i] = localBuckets[i] + 1;
            }
