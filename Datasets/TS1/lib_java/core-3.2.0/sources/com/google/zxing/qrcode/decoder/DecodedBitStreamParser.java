...
/* loaded from: core-3.2.0.jar:com/google/zxing/qrcode/decoder/DecodedBitStreamParser.class */
final class DecodedBitStreamParser {
    ...
    private static void decodeHanziSegment(BitSource bits, StringBuilder result, int count) throws FormatException {
        ...
        while (count > 0) {
            ...
            int assembledTwoBytes2 = i + i2;
            buffer[offset] = (byte) ((assembledTwoBytes2 >> 8) & 255);
            buffer[offset + GB2312_SUBSET] = (byte) (assembledTwoBytes2 & 255);
            offset += 2;
            count--;
        }
        try {
    ...
    private static void decodeKanjiSegment(BitSource bits, StringBuilder result, int count) throws FormatException {
        ...
        while (count > 0) {
            ...
            buffer[offset + GB2312_SUBSET] = (byte) assembledTwoBytes2;
            offset += 2;
            count--;
        }
        try {
    ...
    private static void decodeAlphanumericSegment(BitSource bits, StringBuilder result, int count, boolean fc1InEffect) throws FormatException {
        ...
        if (fc1InEffect) {
            for (int i = start; i < result.length(); i += GB2312_SUBSET) {
                if (result.charAt(i) == '%') {
                    if (i < result.length() - GB2312_SUBSET && result.charAt(i + GB2312_SUBSET) == '%') {
