...
/* loaded from: core-3.2.0.jar:com/google/zxing/qrcode/detector/FinderPatternFinder.class */
public class FinderPatternFinder {
                ...
                if (this.image.get(j, i2)) {
                    ...
                    if (currentState == 4) {
                        if (foundPatternCross(stateCount)) {
                            boolean confirmed = handlePossibleCenter(stateCount, i2, j, pureBarcode);
                            if (confirmed) {
                                iSkip = CENTER_QUORUM;
                                if (this.hasSkipped) {
                                    ...
                                    if (rowSkip > stateCount[CENTER_QUORUM]) {
                                        i2 += (rowSkip - stateCount[CENTER_QUORUM]) - iSkip;
                                        j = maxJ - 1;
                                    }
                                }
                                currentState = 0;
                    ...
                    int i4 = currentState;
                    stateCount[i4] = stateCount[i4] + 1;
                }
                j++;
            }
            if (foundPatternCross(stateCount)) {
                boolean confirmed2 = handlePossibleCenter(stateCount, i2, maxJ, pureBarcode);
                if (confirmed2) {
                    iSkip = stateCount[0];
                    if (this.hasSkipped) {
                        done = haveMultiplyConfirmedCenters();
    ...
    private static float centerFromEnd(int[] stateCount, int end) {
        return ((end - stateCount[4]) - stateCount[3]) - (stateCount[CENTER_QUORUM] / 2.0f);
    }
    ...
    protected static boolean foundPatternCross(int[] stateCount) {
        ...
        return Math.abs(moduleSize - ((float) stateCount[0])) < maxVariance && Math.abs(moduleSize - ((float) stateCount[1])) < maxVariance && Math.abs((3.0f * moduleSize) - ((float) stateCount[CENTER_QUORUM])) < 3.0f * maxVariance && Math.abs(moduleSize - ((float) stateCount[3])) < maxVariance && Math.abs(moduleSize - ((float) stateCount[4])) < maxVariance;
    }
    ...
    private boolean crossCheckDiagonal(int startI, int centerJ, int maxCount, int originalStateCountTotal) {
        ...
        int stateCountTotal = stateCount[0] + stateCount[1] + stateCount[CENTER_QUORUM] + stateCount[3] + stateCount[4];
        return Math.abs(stateCountTotal - originalStateCountTotal) < CENTER_QUORUM * originalStateCountTotal && foundPatternCross(stateCount);
    }
    ...
    private float crossCheckVertical(int startI, int centerJ, int maxCount, int originalStateCountTotal) {
        ...
        int stateCountTotal = stateCount[0] + stateCount[1] + stateCount[CENTER_QUORUM] + stateCount[3] + stateCount[4];
        if (5 * Math.abs(stateCountTotal - originalStateCountTotal) < CENTER_QUORUM * originalStateCountTotal && foundPatternCross(stateCount)) {
    ...
    private float crossCheckHorizontal(int startJ, int centerI, int maxCount, int originalStateCountTotal) {
        ...
        int stateCountTotal = stateCount[0] + stateCount[1] + stateCount[CENTER_QUORUM] + stateCount[3] + stateCount[4];
        if (5 * Math.abs(stateCountTotal - originalStateCountTotal) < originalStateCountTotal && foundPatternCross(stateCount)) {
    ...
    protected final boolean handlePossibleCenter(int[] stateCount, int i, int j, boolean pureBarcode) {
        ...
        if (!Float.isNaN(centerI)) {
            float centerJ2 = crossCheckHorizontal((int) centerJ, (int) centerI, stateCount[CENTER_QUORUM], stateCountTotal);
            if (!Float.isNaN(centerJ2)) {
                if (!pureBarcode || crossCheckDiagonal((int) centerI, (int) centerJ2, stateCount[CENTER_QUORUM], stateCountTotal)) {
    ...
    private FinderPattern[] selectBestPatterns() throws NotFoundException {
        ...
        return new FinderPattern[]{this.possibleCenters.get(0), this.possibleCenters.get(1), this.possibleCenters.get(CENTER_QUORUM)};
    }

    /* loaded from: core-3.2.0.jar:com/google/zxing/qrcode/detector/FinderPatternFinder$FurthestFromAverageComparator.class */
    ...
    /* loaded from: core-3.2.0.jar:com/google/zxing/qrcode/detector/FinderPatternFinder$CenterComparator.class */
