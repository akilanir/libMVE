...
/* loaded from: core-3.2.0.jar:com/google/zxing/maxicode/decoder/DecodedBitStreamParser.class */
final class DecodedBitStreamParser {
        ...
        switch (mode) {
            ...
            case PDF417Common.MIN_ROWS_IN_BARCODE /* 3 */:
                ...
                if (result.toString().startsWith("[)>\u001e01\u001d")) {
                    result.insert(9, postcode + (char) 29 + country + (char) 29 + service + (char) 29);
                    break;
                } else {
                    result.insert(0, postcode + (char) 29 + country + (char) 29 + service + (char) 29);
                    break;
                }
            case 4:
    ...
    private static int getPostCode2(byte[] bytes) {
        return getInt(bytes, new byte[]{33, 34, 35, 36, 25, 26, 27, FS, GS, RS, 19, 20, 21, 22, 23, 24, 13, 14, 15, 16, 17, 18, 7, 8, 9, 10, 11, 12, 1, 2});
    }

    private static String getPostCode3(byte[] bytes) {
        return String.valueOf(new char[]{SETS[0].charAt(getInt(bytes, new byte[]{39, 40, 41, 42, 31, 32})), SETS[0].charAt(getInt(bytes, new byte[]{33, 34, 35, 36, 25, 26})), SETS[0].charAt(getInt(bytes, new byte[]{27, FS, GS, RS, 19, 20})), SETS[0].charAt(getInt(bytes, new byte[]{21, 22, 23, 24, 13, 14})), SETS[0].charAt(getInt(bytes, new byte[]{15, 16, 17, 18, 7, 8})), SETS[0].charAt(getInt(bytes, new byte[]{9, 10, 11, 12, 1, 2}))});
    }
