...
/* loaded from: core-3.2.0.jar:com/google/zxing/aztec/detector/Detector.class */
public final class Detector {
    ...
    private static int getRotation(int[] sides, int length) throws NotFoundException {
        ...
        for (int shift = 0; shift < 4; shift++) {
            if (Integer.bitCount(cornerBits2 ^ EXPECTED_CORNER_BITS[shift]) <= 2) {
                return shift;
            }
        }
        throw NotFoundException.getNotFoundInstance();
    ...
    private ResultPoint[] getMatrixCornerPoints(ResultPoint[] bullsEyeCorners) {
        return expandSquare(bullsEyeCorners, 2 * this.nbCenterLayers, getDimension());
    }
    ...
    private Point getFirstDifferent(Point init, boolean color, int dx, int dy) {
        ...
        int x2 = x - dx;
        int y3 = y - dy;
        while (isValid(x2, y3) && this.image.get(x2, y3) == color) {
            x2 += dx;
        }
        int x3 = x2 - dx;
        while (isValid(x3, y3) && this.image.get(x3, y3) == color) {
            y3 += dy;
        }
        return new Point(x3, y3 - dy);
    }
    ...
    private static ResultPoint[] expandSquare(ResultPoint[] cornerPoints, float oldSide, float newSide) {
        ...
        ResultPoint result1 = new ResultPoint(centerx2 + (ratio * dx2), centery2 + (ratio * dy2));
    ...
    /* loaded from: core-3.2.0.jar:com/google/zxing/aztec/detector/Detector$Point.class */
