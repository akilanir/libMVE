...
/* loaded from: com.google.android.exoplayer.exoplayer.r1.5.5.jar:com/google/android/exoplayer/extractor/DefaultExtractorInput.class */
public final class DefaultExtractorInput implements ExtractorInput {
    ...
    public boolean readFully(byte[] target, int offset, int length, boolean allowEndOfInput) throws IOException, InterruptedException {
        int bytesRead;
        int readFromPeekBuffer = readFromPeekBuffer(target, offset, length);
        while (true) {
            bytesRead = readFromPeekBuffer;
            if (bytesRead >= length || bytesRead == -1) {
                break;
            }
            readFromPeekBuffer = readFromDataSource(target, offset, length, bytesRead, allowEndOfInput);
        }
        commitBytesRead(bytesRead);
        return bytesRead != -1;
    }
    ...
    public boolean skipFully(int length, boolean allowEndOfInput) throws IOException, InterruptedException {
        int bytesSkipped;
        int skipFromPeekBuffer = skipFromPeekBuffer(length);
        while (true) {
            bytesSkipped = skipFromPeekBuffer;
            if (bytesSkipped >= length || bytesSkipped == -1) {
                break;
            }
            skipFromPeekBuffer = readFromDataSource(SCRATCH_SPACE, -bytesSkipped, Math.min(length, bytesSkipped + SCRATCH_SPACE.length), bytesSkipped, allowEndOfInput);
        }
        commitBytesRead(bytesSkipped);
