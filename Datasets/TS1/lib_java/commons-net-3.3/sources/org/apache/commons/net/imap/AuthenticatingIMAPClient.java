...
/* loaded from: commons-net-3.3.jar:org/apache/commons/net/imap/AuthenticatingIMAPClient.class */
public class AuthenticatingIMAPClient extends IMAPSClient {
    ...
    public boolean auth(AUTH_METHOD method, String username, String password) throws IOException, NoSuchAlgorithmException, InvalidKeyException, InvalidKeySpecException {
        ...
        switch (method) {
            case PLAIN:
                int result = sendData(Base64.encodeBase64StringUnChunked(("��" + username + "��" + password).getBytes(getCharsetName())));
                if (result == 0) {
            ...
            case LOGIN:
                if (sendData(Base64.encodeBase64StringUnChunked(username.getBytes(getCharsetName()))) == 3) {
                    int result3 = sendData(Base64.encodeBase64StringUnChunked(password.getBytes(getCharsetName())));
                    if (result3 == 0) {
                        setState(IMAP.IMAPState.AUTH_STATE);
                    }
                    if (result3 == 0) {
    ...
    /* loaded from: commons-net-3.3.jar:org/apache/commons/net/imap/AuthenticatingIMAPClient$AUTH_METHOD.class */
