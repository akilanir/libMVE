...
/* loaded from: slf4j-api-1.7.7.jar:org/slf4j/LoggerFactory.class */
public final class LoggerFactory {
    ...
    private static final void versionSanityCheck() {
        try {
            ...
            boolean match = false;
            for (int i = 0; i < API_COMPATIBILITY_LIST.length; i += ONGOING_INITIALIZATION) {
                if (requested.startsWith(API_COMPATIBILITY_LIST[i])) {
                    match = ONGOING_INITIALIZATION;
                }
            }
            if (!match) {
    ...
    private static boolean isAmbiguousStaticLoggerBinderPathSet(Set<URL> staticLoggerBinderPathSet) {
        return staticLoggerBinderPathSet.size() > ONGOING_INITIALIZATION;
    }
