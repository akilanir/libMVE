...
/* loaded from: mongo-java-driver-2.12.5.jar:com/mongodb/DB.class */
public abstract class DB {
    ...
    public DBCollection getCollectionFromString(String s) {
        ...
        while (true) {
            ...
            s = s.substring(idx + 1);
            if (foo == null) {
                collection = getCollection(b);
            } else {
                collection = foo.getCollection(b);
            }
            foo = collection;
            indexOf = s.indexOf(".");
        }
    ...
    private DBObject wrapCommand(DBObject cmd, ReadPreference readPreference) {
        if (getMongo().isMongosConnection() && !ReadPreference.primary().equals(readPreference) && !ReadPreference.secondaryPreferred().equals(readPreference) && (cmd instanceof BasicDBObject)) {
            cmd = new BasicDBObject("$query", cmd).append("$readPreference", (Object) readPreference.toDBObject());
        }
        return cmd;
    }
    ...
    /* loaded from: mongo-java-driver-2.12.5.jar:com/mongodb/DB$CommandResultPair.class */
    ...
    private boolean isValidName(String dbname) {
        return (dbname.length() == 0 || dbname.contains(" ")) ? false : true;
    }
