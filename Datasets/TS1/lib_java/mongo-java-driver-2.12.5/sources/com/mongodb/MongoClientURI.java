...
/* loaded from: mongo-java-driver-2.12.5.jar:com/mongodb/MongoClientURI.class */
        ...
        try {
            ...
            if (idx3 > 0) {
                ...
                if (idx4 == -1) {
                    ...
                    userName = URLDecoder.decode(authPart.substring(0, idx4), UTF_8);
                    password = URLDecoder.decode(authPart.substring(idx4 + 1), UTF_8).toCharArray();
                }
            }
    ...
    private MongoCredential createCredentials(Map<String, List<String>> optionsMap, String userName, char[] password, String database) {
        ...
        if (mechanism.equals("GSSAPI")) {
            ...
            if (gssapiServiceName != null) {
                gssapiCredential = gssapiCredential.withMechanismProperty(DBPort.GSSAPIAuthenticator.SERVICE_NAME_KEY, gssapiServiceName);
            }
