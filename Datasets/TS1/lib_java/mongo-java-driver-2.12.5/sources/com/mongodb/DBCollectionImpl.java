...
/* loaded from: mongo-java-driver-2.12.5.jar:com/mongodb/DBCollectionImpl.class */
class DBCollectionImpl extends DBCollection {
    ...
    BulkWriteResult executeBulkWriteOperation(boolean ordered, List<WriteRequest> writeRequests, WriteConcern writeConcern, DBEncoder encoder) {
        ...
        WriteConcern writeConcern2 = writeConcern.continueOnError(!ordered);
        if (encoder == null) {
            encoder = DefaultDBEncoder.FACTORY.create();
        }
        DBPort port = this.db.getConnector().getPrimaryPort();
        try {
            BulkWriteBatchCombiner bulkWriteBatchCombiner = new BulkWriteBatchCombiner(port.getAddress(), writeConcern2);
            for (Run run : getRunGenerator(ordered, writeRequests, writeConcern2, encoder, port)) {
                try {
            ...
            BulkWriteResult result2 = bulkWriteBatchCombiner.getResult();
    ...
    protected WriteResult insert(List<DBObject> list, boolean shouldApply, WriteConcern concern, DBEncoder encoder) {
        ...
        try {
            if (useWriteCommands(concern, port)) {
                try {
                    WriteResult translateBulkWriteResult = translateBulkWriteResult(insertWithCommandProtocol(list, concern, encoder, port, shouldApply), WriteRequest.Type.INSERT, concern, port.getAddress());
                    this.db.getConnector().releasePort(port);
            ...
            WriteResult insertWithWriteProtocol = insertWithWriteProtocol(list, concern, encoder, port, shouldApply);
            this.db.getConnector().releasePort(port);
    ...
    public WriteResult remove(DBObject query, boolean multi, WriteConcern concern, DBEncoder encoder) {
        ...
        try {
            if (useWriteCommands(concern, port)) {
                try {
                    WriteResult translateBulkWriteResult = translateBulkWriteResult(removeWithCommandProtocol(Arrays.asList(new RemoveRequest(query, multi)), concern, encoder, port), WriteRequest.Type.REMOVE, concern, port.getAddress());
                    this.db.getConnector().releasePort(port);
                    return translateBulkWriteResult;
                } catch (BulkWriteException e) {
            ...
            WriteResult say = this.db.getConnector().say(this._db, OutMessage.remove(this, encoder, query, multi), concern, port);
            this.db.getConnector().releasePort(port);
            return say;
        } catch (Throwable th) {
            this.db.getConnector().releasePort(port);
        ...
        try {
            if (useWriteCommands(concern, port)) {
                try {
                    ...
                    WriteResult translateBulkWriteResult = translateBulkWriteResult(bulkWriteResult, WriteRequest.Type.UPDATE, concern, port.getAddress());
                    this.db.getConnector().releasePort(port);
                    return translateBulkWriteResult;
                } catch (BulkWriteException e) {
            ...
            WriteResult say = this.db.getConnector().say(this._db, OutMessage.update(this, encoder, upsert, multi, query, o), concern, port);
            this.db.getConnector().releasePort(port);
            return say;
        } catch (Throwable th) {
            this.db.getConnector().releasePort(port);
    ...
    private BulkWriteResult writeWithCommandProtocol(final DBPort port, final WriteRequest.Type type, final BaseWriteCommandMessage message, final WriteConcern writeConcern) {
        return (BulkWriteResult) this.db.getConnector().doOperation(this.db, port, new DBPort.Operation<BulkWriteResult>() { // from class: com.mongodb.DBCollectionImpl.2
            /* JADX WARN: Can't rename method to resolve collision */
            @Override // com.mongodb.DBPort.Operation
            public BulkWriteResult execute() throws IOException {
                ...
                do {
                    batchNum++;
                    BaseWriteCommandMessage nextMessage = DBCollectionImpl.this.sendWriteCommandMessage(curMessage, batchNum, port);
                    int itemCount = nextMessage != null ? curMessage.getItemCount() - nextMessage.getItemCount() : curMessage.getItemCount();
                    IndexMap indexMap = IndexMap.create(currentRangeStartIndex, itemCount);
                    CommandResult commandResult = DBCollectionImpl.this.receiveWriteCommandMessage(port);
                    if ((DBCollectionImpl.this.willTrace() && nextMessage != null) || batchNum > 1) {
                        DBCollectionImpl.this.getLogger().fine(String.format("Received response for batch %d", Integer.valueOf(batchNum)));
                    }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public BaseWriteCommandMessage sendWriteCommandMessage(BaseWriteCommandMessage message, int batchNum, DBPort port) throws IOException {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public CommandResult receiveWriteCommandMessage(DBPort port) throws IOException {
    ...
    /* loaded from: mongo-java-driver-2.12.5.jar:com/mongodb/DBCollectionImpl$OrderedRunGenerator.class */
    private class OrderedRunGenerator implements Iterable<Run> {
        ...
        public Iterator<Run> iterator() {
            return new Iterator<Run>() { // from class: com.mongodb.DBCollectionImpl.OrderedRunGenerator.1
                ...
                @Override // java.util.Iterator
                public Run next() {
                    Run run = DBCollectionImpl.this.new Run(((WriteRequest) OrderedRunGenerator.this.writeRequests.get(this.curIndex)).getType(), OrderedRunGenerator.this.writeConcern, OrderedRunGenerator.this.encoder);
    ...
    /* loaded from: mongo-java-driver-2.12.5.jar:com/mongodb/DBCollectionImpl$UnorderedRunGenerator.class */
            ...
            return new Iterator<Run>() { // from class: com.mongodb.DBCollectionImpl.UnorderedRunGenerator.1
                ...
                public Run next() {
                    while (this.curIndex < UnorderedRunGenerator.this.writeRequests.size()) {
                        ...
                        if (run == null) {
                            run = DBCollectionImpl.this.new Run(writeRequest.getType(), UnorderedRunGenerator.this.writeConcern, UnorderedRunGenerator.this.encoder);
    ...
    /* loaded from: mongo-java-driver-2.12.5.jar:com/mongodb/DBCollectionImpl$Run.class */
    private class Run {
        ...
        /* loaded from: mongo-java-driver-2.12.5.jar:com/mongodb/DBCollectionImpl$Run$RunExecutor.class */
