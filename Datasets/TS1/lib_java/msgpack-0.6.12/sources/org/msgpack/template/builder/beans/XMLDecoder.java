...
/* loaded from: msgpack-0.6.12.jar:org/msgpack/template/builder/beans/XMLDecoder.class */
public class XMLDecoder {
    ...
    /* loaded from: msgpack-0.6.12.jar:org/msgpack/template/builder/beans/XMLDecoder$DefaultExceptionListener.class */
    ...
    /* loaded from: msgpack-0.6.12.jar:org/msgpack/template/builder/beans/XMLDecoder$SAXHandler.class */
    private class SAXHandler extends DefaultHandler {
        ...
        public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
            ...
            if ("boolean".equals(qName) || "byte".equals(qName) || "char".equals(qName) || "class".equals(qName) || "double".equals(qName) || "float".equals(qName) || "int".equals(qName) || "long".equals(qName) || "short".equals(qName) || "string".equals(qName) || BeansUtils.NULL.equals(qName)) {
        ...
        private Object executeCommon(Elem elem) throws Exception {
            ...
            if (elem.fromProperty) {
                method = (args.size() == 0 ? BeansUtils.GET : BeansUtils.SET) + capitalize(method);
            }
            if (elem.fromIndex) {
                Integer index = Integer.valueOf(method);
                args.add(0, index);
                method = args.size() == 1 ? BeansUtils.GET : BeansUtils.SET;
            }
            if (elem.fromField) {
                Field f = ((Class) elem.target).getField(method);
                return new Expression(f, BeansUtils.GET, new Object[]{null}).getValue();
            }
            if (elem.fromOwner) {
            ...
            if (elem.target == XMLDecoder.this.owner) {
                ...
                try {
                    ...
                    Method mostSpecificMethod = findMethod(XMLDecoder.this.owner instanceof Class ? (Class) XMLDecoder.this.owner : XMLDecoder.this.owner.getClass(), method, c);
        ...
        private Method findMethod(Class<?> clazz, String methodName, Class<?>[] clazzes) throws Exception {
            ...
            int methodCounter = 1;
            Iterator i$ = matchMethods.iterator();
            while (i$.hasNext()) {
                Method method2 = i$.next();
                int difference = comparator.compare(chosenOne, method2);
                if (difference > 0) {
                    chosenOne = method2;
                    methodCounter = 1;
    ...
    /* loaded from: msgpack-0.6.12.jar:org/msgpack/template/builder/beans/XMLDecoder$Elem.class */
