package com.google.common.net;

import com.google.common.annotations.GwtCompatible;
import com.google.common.base.Joiner;
import com.google.common.collect.ImmutableSet;
import com.google.common.collect.Lists;
import java.util.List;

@GwtCompatible
/* loaded from: guava-15.0.jar:com/google/common/net/TrieParser.class */
class TrieParser {
    private static final Joiner PREFIX_JOINER = Joiner.on("");

    TrieParser() {
    }

    static ImmutableSet<String> parseTrie(CharSequence encoded) {
        ImmutableSet.Builder<String> builder = ImmutableSet.builder();
        int encodedLen = encoded.length();
        int i = 0;
        while (true) {
            int idx = i;
            if (idx < encodedLen) {
                i = idx + doParseTrieToBuilder(Lists.newLinkedList(), encoded.subSequence(idx, encodedLen), builder);
            } else {
                return builder.build();
            }
        }
    }

    private static int doParseTrieToBuilder(List<CharSequence> stack, CharSequence encoded, ImmutableSet.Builder<String> builder) {
        int encodedLen = encoded.length();
        int idx = 0;
        char c = 0;
        while (idx < encodedLen) {
            c = encoded.charAt(idx);
            if (c == '&' || c == '?' || c == '!') {
                break;
            }
            idx++;
        }
        stack.add(0, reverse(encoded.subSequence(0, idx)));
        if (c == '!' || c == '?') {
            String domain = PREFIX_JOINER.join(stack);
            if (domain.length() > 0) {
                builder.add((ImmutableSet.Builder<String>) domain);
            }
        }
        int idx2 = idx + 1;
        if (c != '?') {
            while (true) {
                if (idx2 >= encodedLen) {
                    break;
                }
                idx2 += doParseTrieToBuilder(stack, encoded.subSequence(idx2, encodedLen), builder);
                if (encoded.charAt(idx2) == '?') {
                    idx2++;
                    break;
                }
            }
        }
        stack.remove(0);
        return idx2;
    }

    private static CharSequence reverse(CharSequence s) {
        int length = s.length();
        if (length <= 1) {
            return s;
        }
        char[] buffer = new char[length];
        buffer[0] = s.charAt(length - 1);
        for (int i = 1; i < length; i++) {
            buffer[i] = s.charAt((length - 1) - i);
            if (Character.isSurrogatePair(buffer[i], buffer[i - 1])) {
                swap(buffer, i - 1, i);
            }
        }
        return new String(buffer);
    }

    private static void swap(char[] buffer, int f, int s) {
        char tmp = buffer[f];
        buffer[f] = buffer[s];
        buffer[s] = tmp;
    }
}
