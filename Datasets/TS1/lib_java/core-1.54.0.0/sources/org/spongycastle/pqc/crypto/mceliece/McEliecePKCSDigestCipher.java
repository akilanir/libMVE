...
/* loaded from: core-1.54.0.0.jar:org/spongycastle/pqc/crypto/mceliece/McEliecePKCSDigestCipher.class */
public class McEliecePKCSDigestCipher {
    ...
    public void init(boolean forEncrypting, CipherParameters param) {
        ...
        reset();
        this.mcElieceCipher.init(forEncrypting, param);
    }
    ...
    public byte[] messageEncrypt() {
        ...
        byte[] enc = null;
        try {
            enc = this.mcElieceCipher.messageEncrypt(hash);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return enc;
    ...
    public byte[] messageDecrypt(byte[] ciphertext) {
        ...
        try {
            output = this.mcElieceCipher.messageDecrypt(ciphertext);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return output;
