...
/* loaded from: core-1.54.0.0.jar:org/spongycastle/asn1/eac/EACTags.class */
public class EACTags {
    ...
    public static int encodeTag(ASN1ApplicationSpecific spec) {
        ...
        if (tag > 31) {
            ...
            retValue = ((retValue2 | 31) << 8) | currentByte;
            while (true) {
                tag >>= 7;
                if (tag <= 0) {
                    break;
                }
                retValue = (retValue | 128) << 8;
                int i = tag & CertificateBody.profileType;
            }
        } else {
            retValue = retValue2 | tag;
        }
        return retValue;
    }
