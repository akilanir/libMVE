...
/* loaded from: core-1.54.0.0.jar:org/spongycastle/math/raw/Mont256.class */
public abstract class Mont256 {
    ...
    public static void multAdd(int[] x, int[] y, int[] z, int[] m, int mInv32) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            long prod2 = t * (m[0] & M);
            long carry2 = ((carry + (prod2 & M)) >>> 32) + (prod1 >>> 32) + (prod2 >>> 32);
            for (int j = 1; j < 8; j++) {
                long prod12 = x_i * (y[j] & M);
                long prod22 = t * (m[j] & M);
                long carry3 = carry2 + (prod12 & M) + (prod22 & M) + (z[j] & M);
                z[j - 1] = (int) carry3;
                carry2 = (carry3 >>> 32) + (prod12 >>> 32) + (prod22 >>> 32);
            }
            long carry4 = carry2 + (z_8 & M);
            z[7] = (int) carry4;
            z_8 = (int) (carry4 >>> 32);
        }
    ...
    public static void multAddXF(int[] x, int[] y, int[] z, int[] m) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            for (int j = 1; j < 8; j++) {
                ...
                long prod2 = t * (m[j] & M);
                long carry3 = carry2 + (prod1 & M) + (prod2 & M) + (z[j] & M);
                z[j - 1] = (int) carry3;
    ...
    public static void reduce(int[] z, int[] m, int mInv32) {
        for (int i = 0; i < 8; i++) {
            ...
            long carry2 = carry >>> 32;
            for (int j = 1; j < 8; j++) {
                long carry3 = carry2 + (t * (m[j] & M)) + (z[j] & M);
                z[j - 1] = (int) carry3;
                carry2 = carry3 >>> 32;
            }
            z[7] = (int) carry2;
    ...
    public static void reduceXF(int[] z, int[] m) {
        for (int i = 0; i < 8; i++) {
            ...
            long carry = t;
            for (int j = 1; j < 8; j++) {
                long carry2 = carry + (t * (m[j] & M)) + (z[j] & M);
                z[j - 1] = (int) carry2;
                carry = carry2 >>> 32;
            }
            z[7] = (int) carry;
