...
/* loaded from: core-1.54.0.0.jar:org/spongycastle/crypto/engines/RijndaelEngine.class */
public class RijndaelEngine implements BlockCipher {
    ...
    private byte mul0x2(int b) {
        if (b != 0) {
            return aLogtable[25 + (logtable[b] & 255)];
        }
        return (byte) 0;
    }
    ...
    private byte mul0x3(int b) {
        if (b != 0) {
            return aLogtable[1 + (logtable[b] & 255)];
        }
        return (byte) 0;
    }
    ...
    private long[][] generateWorkingKey(byte[] key) {
        ...
        while (j < KC && t < (this.ROUNDS + 1) * (this.BC / 8)) {
            for (int i3 = 0; i3 < 4; i3++) {
                ...
                int i4 = i3;
                jArr[i4] = jArr[i4] | ((tk[i3][j] & 255) << ((t * 8) % this.BC));
            }
            j++;
            t++;
        }
        while (t < (this.ROUNDS + 1) * (this.BC / 8)) {
            for (int i5 = 0; i5 < 4; i5++) {
                byte[] bArr = tk[i5];
                bArr[0] = (byte) (bArr[0] ^ S[tk[(i5 + 1) % 4][KC - 1] & 255]);
            }
            byte[] bArr2 = tk[0];
            int i6 = rconpointer;
            rconpointer++;
            bArr2[0] = (byte) (bArr2[0] ^ rcon[i6]);
            if (KC <= 6) {
                for (int j2 = 1; j2 < KC; j2++) {
                    for (int i7 = 0; i7 < 4; i7++) {
                        byte[] bArr3 = tk[i7];
                        int i8 = j2;
                        bArr3[i8] = (byte) (bArr3[i8] ^ tk[i7][j2 - 1]);
                    }
                }
            } else {
                for (int j3 = 1; j3 < 4; j3++) {
                    for (int i9 = 0; i9 < 4; i9++) {
                        byte[] bArr4 = tk[i9];
                        int i10 = j3;
                        bArr4[i10] = (byte) (bArr4[i10] ^ tk[i9][j3 - 1]);
                    }
                }
                for (int i11 = 0; i11 < 4; i11++) {
                    byte[] bArr5 = tk[i11];
                    bArr5[4] = (byte) (bArr5[4] ^ S[tk[i11][3] & 255]);
                }
                for (int j4 = 5; j4 < KC; j4++) {
                    for (int i12 = 0; i12 < 4; i12++) {
                        byte[] bArr6 = tk[i12];
                        int i13 = j4;
                        bArr6[i13] = (byte) (bArr6[i13] ^ tk[i12][j4 - 1]);
                    }
                }
            }
            int j5 = 0;
            while (j5 < KC && t < (this.ROUNDS + 1) * (this.BC / 8)) {
                for (int i14 = 0; i14 < 4; i14++) {
                    ...
                    int i15 = i14;
                    jArr2[i15] = jArr2[i15] | ((tk[i14][j5] & 255) << ((t * 8) % this.BC));
                }
                j5++;
                t++;
            }
    ...
    private void unpackBlock(byte[] bytes, int off) {
        ...
        this.A1 = bytes[index] & 255;
        this.A2 = bytes[r10] & 255;
        int index2 = index + 1 + 1 + 1;
        this.A3 = bytes[r10] & 255;
        for (int j = 8; j != this.BC; j += 8) {
            int i = index2;
            this.A0 |= (bytes[i] & 255) << j;
            this.A1 |= (bytes[r10] & 255) << j;
            this.A2 |= (bytes[r10] & 255) << j;
            index2 = index2 + 1 + 1 + 1 + 1;
            this.A3 |= (bytes[r10] & 255) << j;
        }
    }
