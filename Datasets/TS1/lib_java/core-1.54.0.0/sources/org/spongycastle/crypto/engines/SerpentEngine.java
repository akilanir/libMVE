...
/* loaded from: core-1.54.0.0.jar:org/spongycastle/crypto/engines/SerpentEngine.class */
public final class SerpentEngine extends SerpentEngineBase {
    ...
    protected int[] makeWorkingKey(byte[] key) throws IllegalArgumentException {
        ...
        while (off + 4 < key.length) {
            int i = length;
            length++;
            kPad[i] = Pack.littleEndianToInt(key, off);
            off += 4;
        }
        if (off % 4 == 0) {
            int i2 = length;
            int length2 = length + 1;
            kPad[i2] = Pack.littleEndianToInt(key, off);
            if (length2 < 8) {
                kPad[length2] = 1;
            }
