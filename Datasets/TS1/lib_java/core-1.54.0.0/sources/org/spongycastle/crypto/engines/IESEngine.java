...
/* loaded from: core-1.54.0.0.jar:org/spongycastle/crypto/engines/IESEngine.class */
public class IESEngine {
    ...
    private byte[] decryptBlock(byte[] in_enc, int inOff, int inLen) throws InvalidCipherTextException {
        ...
        if (this.cipher == null) {
            ...
            System.arraycopy(K3, K12.length, K2, 0, K2.length);
            if (this.IV != null) {
                this.cipher.init(false, new ParametersWithIV(new KeyParameter(K12), this.IV));
            } else {
                this.cipher.init(false, new KeyParameter(K12));
            }
            M = new byte[this.cipher.getOutputSize((inLen - this.V.length) - this.mac.getMacSize())];
            int len2 = this.cipher.processBytes(in_enc, inOff + this.V.length, (inLen - this.V.length) - this.mac.getMacSize(), M, 0);
    ...
    protected byte[] getLengthTag(byte[] p2) {
        byte[] L2 = new byte[8];
        if (p2 != null) {
            Pack.longToBigEndian(p2.length * 8, L2, 0);
        }
        return L2;
    }
}
