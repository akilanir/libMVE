...
/* loaded from: core-1.54.0.0.jar:org/spongycastle/crypto/engines/RC6Engine.class */
public class RC6Engine implements BlockCipher {
    ...
    private void setKey(byte[] key) {
        ...
        for (int k = 0; k < iter; k++) {
            int rotateLeft = rotateLeft(this._S[i3] + A + B, 3);
            this._S[i3] = rotateLeft;
            A = rotateLeft;
    ...
    private int encryptBlock(byte[] in, int inOff, byte[] out, int outOff) {
        ...
        int D2 = D + this._S[1];
        for (int i = 1; i <= 20; i++) {
            int t = rotateLeft(B2 * ((2 * B2) + 1), 5);
            int u = rotateLeft(D2 * ((2 * D2) + 1), 5);
            int A2 = rotateLeft(A ^ t, u) + this._S[2 * i];
            A = B2;
            B2 = rotateLeft(C ^ u, t) + this._S[(2 * i) + 1];
            C = D2;
            D2 = A2;
        }
        int A3 = A + this._S[42];
    ...
    private int decryptBlock(byte[] in, int inOff, byte[] out, int outOff) {
        ...
        for (int i = 20; i >= 1; i--) {
            ...
            C2 = rotateRight(C3 - this._S[(2 * i) + 1], t) ^ u;
            int A3 = temp - this._S[2 * i];
