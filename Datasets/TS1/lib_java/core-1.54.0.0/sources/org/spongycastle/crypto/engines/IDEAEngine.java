...
/* loaded from: core-1.54.0.0.jar:org/spongycastle/crypto/engines/IDEAEngine.class */
public class IDEAEngine implements BlockCipher {
    ...
    private void ideaFunc(int[] workingKey, byte[] in, int inOff, byte[] out, int outOff) {
        ...
        for (int round = 0; round < 8; round++) {
            ...
            int x24 = mul(x23, workingKey[keyOff5]);
            keyOff = keyOff6 + 1;
            int x14 = mul((x13 + x24) & MASK, workingKey[keyOff6]);
            int x25 = (x24 + x14) & MASK;
            x0 = x02 ^ x14;
            x3 = x32 ^ x25;
            x1 = x14 ^ x22;
            x2 = x25 ^ x12;
        ...
        wordToBytes(mul(x0, workingKey[i2]), out, outOff);
        int keyOff8 = keyOff7 + 1;
        wordToBytes(x2 + workingKey[keyOff7], out, outOff + 2);
        wordToBytes(x1 + workingKey[keyOff8], out, outOff + 4);
