...
public class EventBus {
        ...
        for (Map.Entry<Class<?>, Collection<EventSubscriber>> entry : methodsInListener.asMap().entrySet()) {
            ...
            try {
                ...
                if (!currentSubscribers.containsAll(eventMethodsInListener)) {
                    ...
                    throw new IllegalArgumentException(new StringBuilder(65 + valueOf.length()).append("missing event subscriber for an annotated method. Is ").append(valueOf).append(" registered?").toString());
                }
    ...
    private static final class LoggingSubscriberExceptionHandler implements SubscriberExceptionHandler {
        ...
        public LoggingSubscriberExceptionHandler(String identifier) {
            ...
            this.logger = Logger.getLogger(new StringBuilder(1 + valueOf.length() + valueOf2.length()).append(valueOf).append(".").append(valueOf2).toString());
        }
        ...
        public void handleException(Throwable exception, SubscriberExceptionContext context) {
            ...
            logger.log(level, new StringBuilder(30 + valueOf.length() + valueOf2.length()).append("Could not dispatch event: ").append(valueOf).append(" to ").append(valueOf2).toString(), exception.getCause());
        }
    }
