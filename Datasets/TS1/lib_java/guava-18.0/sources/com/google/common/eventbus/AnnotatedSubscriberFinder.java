            ...
            for (Method superClazzMethod : arr$) {
                if (superClazzMethod.isAnnotationPresent(Subscribe.class) && !superClazzMethod.isBridge()) {
                    ...
                    if (parameterTypes.length != 1) {
                        ...
                        throw new IllegalArgumentException(new StringBuilder(128 + valueOf.length()).append("Method ").append(valueOf).append(" has @Subscribe annotation, but requires ").append(parameterTypes.length).append(" arguments.  Event subscriber methods must require a single argument.").toString());
                    }
