...
public final class Iterators {
    ...
    public static <T> T getOnlyElement(Iterator<T> iterator) {
        ...
        sb.append(new StringBuilder(31 + valueOf.length()).append("expected one element but was: <").append(valueOf).toString());
        for (int i = 0; i < 4 && iterator.hasNext(); i++) {
            String valueOf2 = String.valueOf(String.valueOf(iterator.next()));
            sb.append(new StringBuilder(2 + valueOf2.length()).append(", ").append(valueOf2).toString());
        }
        if (iterator.hasNext()) {
            sb.append(", ...");
        }
