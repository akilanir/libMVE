...
public class ImmutableRangeMap<K extends Comparable<?>, V> implements RangeMap<K, V> {
            ...
            if (!this.keyRanges.complement().encloses(range)) {
                for (Map.Entry<Range<K>, V> entry : this.rangeMap.asMapOfRanges().entrySet()) {
                    ...
                    if (key.isConnected(range) && !key.intersection(range).isEmpty()) {
                        ...
                        throw new IllegalArgumentException(new StringBuilder(47 + valueOf.length() + valueOf2.length()).append("Overlapping ranges: range ").append(valueOf).append(" overlaps with entry ").append(valueOf2).toString());
                    }
                }
            }
    ...
    public ImmutableRangeMap<K, V> subRangeMap(final Range<K> range) {
        ...
        final int binarySearch = SortedLists.binarySearch(this.ranges, (Function<? super E, Comparable>) Range.upperBoundFn(), range.lowerBound, SortedLists.KeyPresentBehavior.FIRST_AFTER, SortedLists.KeyAbsentBehavior.NEXT_HIGHER);
        int binarySearch2 = SortedLists.binarySearch(this.ranges, (Function<? super E, Comparable>) Range.lowerBoundFn(), range.upperBound, SortedLists.KeyPresentBehavior.ANY_PRESENT, SortedLists.KeyAbsentBehavior.NEXT_HIGHER);
