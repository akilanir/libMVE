...
/* loaded from: achartengine-1.2.0.jar:org/achartengine/TouchHandler.class */
public class TouchHandler implements ITouchHandler {
    ...
    public boolean handleTouch(MotionEvent event) {
        ...
        if (this.mRenderer == null || action != 2) {
            ...
            if (event.getPointerCount() > 1 && ((this.oldX2 >= 0.0f || this.oldY2 >= 0.0f) && this.mRenderer.isZoomEnabled())) {
                ...
                if (tan1 <= 0.25d && tan2 <= 0.25d) {
                    float zoomRate2 = newDeltaX / oldDeltaX;
                    applyZoom(zoomRate2, 1);
                } else if (tan1 >= 3.73d && tan2 >= 3.73d) {
                    float zoomRate3 = newDeltaY / oldDeltaY;
                    applyZoom(zoomRate3, 2);
                } else {
    ...
    private void applyZoom(float zoomRate, int axis) {
        ...
        if (this.mPinchZoom != null && zoomRate2 > 0.9d && zoomRate2 < 1.1d) {
