...
/* loaded from: org.osmdroid.osmdroid-android.5.0.1.jar:org/osmdroid/views/overlay/TilesOverlay.class */
public class TilesOverlay extends Overlay implements IOverlayMenuProvider {
    ...
    static final ColorFilter neg = new ColorMatrixColorFilter(negate);
    ...
    protected void draw(Canvas c, MapView osmv, boolean shadow) {
        this.isInvert = osmv.getController().isInvertedTiles();
        if (shadow) {
            return;
        }
    ...
    public boolean onCreateOptionsMenu(Menu pMenu, int pMenuIdOffset, MapView pMapView) {
        ...
        for (int a = 0; a < TileSourceFactory.getTileSources().size(); a++) {
            ...
            mapMenu.add(MENU_MAP_MODE + pMenuIdOffset, MENU_TILE_SOURCE_STARTING_ID + a + pMenuIdOffset, 0, tileSource.name());
        }
