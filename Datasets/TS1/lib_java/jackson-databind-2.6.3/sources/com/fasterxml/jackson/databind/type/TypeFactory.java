...
/* loaded from: jackson-databind-2.6.3.jar:com/fasterxml/jackson/databind/type/TypeFactory.class */
public final class TypeFactory implements Serializable {
    ...
    public JavaType constructSpecializedType(JavaType baseType, Class<?> subclass) {
        ...
        if ((baseType instanceof SimpleType) && (subclass.isArray() || Map.class.isAssignableFrom(subclass) || Collection.class.isAssignableFrom(subclass))) {
            ...
            if (h2 != null) {
                subtype = subtype.withTypeHandler(h2);
            }
            return subtype;
        }
    ...
    public JavaType[] findTypeParameters(JavaType type, Class<?> expType) {
        if (expType == type.getParameterSource()) {
            ...
            for (int i = 0; i < count; i++) {
                result[i] = type.mo2containedType(i);
            }
            return result;
        }
    ...
    public JavaType constructParametrizedType(Class<?> parametrized, Class<?> parametersFor, JavaType... parameterTypes) {
        ...
        if (parametrized.isArray()) {
            ...
            resultType = constructMapType((Class<? extends Map>) parametrized, parameterTypes[0], parameterTypes[1]);
            ...
            resultType = constructCollectionType((Class<? extends Collection>) parametrized, parameterTypes[0]);
        } else {
    ...
    protected JavaType _fromClass(Class<?> clz, TypeBindings context) {
        ...
        if (clz.isArray()) {
            ...
            JavaType[] pts2 = findTypeParameters(clz, Map.Entry.class);
            if (pts2 == null || pts2.length != 2) {
    ...
    protected JavaType _fromParamType(ParameterizedType type, TypeBindings context) {
        ...
        if (Collection.class.isAssignableFrom(rawType)) {
            JavaType subtype2 = constructSimpleType(rawType, rawType, pt);
        ...
        if (AtomicReference.class.isAssignableFrom(rawType)) {
            ...
            if (rawType == AtomicReference.class) {
                ...
                JavaType subtype3 = constructSimpleType(rawType, rawType, pt);
                JavaType[] pts = findTypeParameters(subtype3, AtomicReference.class, context);
        ...
        if (Map.Entry.class.isAssignableFrom(rawType)) {
            ...
            if (rawType == Map.Entry.class) {
                ...
                JavaType subtype4 = constructSimpleType(rawType, rawType, pt);
                JavaType[] pts2 = findTypeParameters(subtype4, Map.Entry.class, context);
                if (pts2 != null && pts2.length == 2) {
                    kt = pts2[0];
                    vt = pts2[1];
                }
            ...
            javaTypeArr[0] = kt == null ? unknownType() : kt;
            javaTypeArr[1] = vt == null ? unknownType() : vt;
            return constructSimpleType(rawType, Map.Entry.class, javaTypeArr);
        }
        if (paramCount == 0) {
    ...
    private JavaType _mapType(Class<?> rawClass) {
        if (rawClass == Properties.class) {
            return MapType.construct(rawClass, (JavaType) CORE_TYPE_STRING, (JavaType) CORE_TYPE_STRING);
        }
