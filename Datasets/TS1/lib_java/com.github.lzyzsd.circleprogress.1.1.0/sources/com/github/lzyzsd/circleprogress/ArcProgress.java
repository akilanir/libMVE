...
/* loaded from: com.github.lzyzsd.circleprogress.1.1.0.jar:com/github/lzyzsd/circleprogress/ArcProgress.class */
public class ArcProgress extends View {
    ...
    protected void initByAttributes(TypedArray attributes) {
        ...
        this.arcAngle = attributes.getDimension(R.styleable.ArcProgress_arc_angle, 288.0f);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        this.rectF.set(this.strokeWidth / 2.0f, this.strokeWidth / 2.0f, View.MeasureSpec.getSize(widthMeasureSpec) - (this.strokeWidth / 2.0f), View.MeasureSpec.getSize(heightMeasureSpec) - (this.strokeWidth / 2.0f));
        float radius = getWidth() / 2.0f;
        float angle = (360.0f - this.arcAngle) / 2.0f;
        this.arcBottomHeight = radius * ((float) (1.0d - Math.cos((angle / 180.0f) * 3.141592653589793d)));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float finishedSweepAngle = (this.progress / getMax()) * this.arcAngle;
        ...
        if (!TextUtils.isEmpty(text)) {
            ...
            canvas.drawText(text, (getWidth() - this.textPaint.measureText(text)) / 2.0f, textBaseline, this.textPaint);
            this.textPaint.setTextSize(this.suffixTextSize);
            float suffixHeight = this.textPaint.descent() + this.textPaint.ascent();
            canvas.drawText(this.suffixText, (getWidth() / 2.0f) + this.textPaint.measureText(text) + this.suffixTextPadding, (textBaseline + textHeight) - suffixHeight, this.textPaint);
        }
        if (!TextUtils.isEmpty(getBottomText())) {
            this.textPaint.setTextSize(this.bottomTextSize);
            float bottomTextBaseline = (getHeight() - this.arcBottomHeight) - ((this.textPaint.descent() + this.textPaint.ascent()) / 2.0f);
            canvas.drawText(getBottomText(), (getWidth() - this.textPaint.measureText(getBottomText())) / 2.0f, bottomTextBaseline, this.textPaint);
        }
    }
