...
/* loaded from: com.android.support.appcompat-v7.22.0.0.jar:android/support/v7/widget/LinearLayoutCompat.class */
public class LinearLayoutCompat extends ViewGroup {
    ...
    /* loaded from: com.android.support.appcompat-v7.22.0.0.jar:android/support/v7/widget/LinearLayoutCompat$DividerMode.class */
    public @interface DividerMode {
    }

    @Retention(RetentionPolicy.SOURCE)
    @IntDef({0, 1})
    /* loaded from: com.android.support.appcompat-v7.22.0.0.jar:android/support/v7/widget/LinearLayoutCompat$OrientationMode.class */
    ...
    void measureHorizontal(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (maxAscent[1] != -1 || maxAscent[0] != -1 || maxAscent[2] != -1 || maxAscent[INDEX_FILL] != -1) {
            int ascent = Math.max(maxAscent[INDEX_FILL], Math.max(maxAscent[0], Math.max(maxAscent[1], maxAscent[2])));
            int descent = Math.max(maxDescent[INDEX_FILL], Math.max(maxDescent[0], Math.max(maxDescent[1], maxDescent[2])));
        ...
        if (skippedMeasure || (delta != 0 && totalWeight > 0.0f)) {
            ...
            this.mTotalLength += getPaddingLeft() + getPaddingRight();
            if (maxAscent[1] != -1 || maxAscent[0] != -1 || maxAscent[2] != -1 || maxAscent[INDEX_FILL] != -1) {
                int ascent2 = Math.max(maxAscent[INDEX_FILL], Math.max(maxAscent[0], Math.max(maxAscent[1], maxAscent[2])));
                int descent2 = Math.max(maxDescent[INDEX_FILL], Math.max(maxDescent[0], Math.max(maxDescent[1], maxDescent[2])));
        ...
        setMeasuredDimension(widthSizeAndState | (childState & (-16777216)), ViewCompat.resolveSizeAndState(Math.max(maxHeight + getPaddingTop() + getPaddingBottom(), getSuggestedMinimumHeight()), heightMeasureSpec, childState << 16));
        if (matchHeight) {
    ...
    /* loaded from: com.android.support.appcompat-v7.22.0.0.jar:android/support/v7/widget/LinearLayoutCompat$LayoutParams.class */
