...
/* loaded from: kolabnotes-java-3.3.4.jar:org/kore/kolab/notes/v3/KolabConfigurationParserV3.class */
public class KolabConfigurationParserV3 implements KolabParser, Serializable {
    ...
    public void write(Object object, OutputStream stream) {
        try {
            ...
            Document document = KolabConfigurationXMLBuilder.createInstance(docBuilder).withIdentification(details.getIdentification()).withAuditInformation(details.getAuditInformation()).withType().withName(details.getTag().getName()).withRelationType().withColor(details.getTag().getColor()).withPriority(details.getTag().getPriority()).withMembers(details.getMembers()).build();
