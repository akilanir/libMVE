...
/* loaded from: rhino-1.7.7.jar:org/mozilla/javascript/tools/idswitch/SwitchGenerator.class */
public class SwitchGenerator {
    ...
    private boolean generate_letter_switch_r(int begin, int end, int L, int indent_level, boolean label_was_defined, boolean inside_if) {
        ...
        if (begin + 1 == end) {
            ...
            if (L > this.char_tail_test_threshold) {
                ...
                for (int i = 1; i != L; i++) {
                    this.P.p(" && ");
                    int column2 = this.columns[i];
                    this.P.p(this.v_s);
                    this.P.p(".charAt(");
                    this.P.p(column2);
                    this.P.p(")==");
                    this.P.qchar(pair.id.charAt(column2));
                }
                this.P.p(") {");
                this.P.p(this.v_id);
                this.P.p("=");
        ...
        while (true) {
            i2++;
            if (i2 != end) {
                int charAt = this.pairs[i2].id.charAt(max_column);
                ch = charAt;
                if (charAt == cur_ch) {
                    continue;
                }
            ...
            boolean after_unreachable = generate_letter_switch_r(same_char_begin, i2, L - 1, next_indent, label_was_defined, use_if);
            if (use_if) {
                this.P.p("}");
    ...
    private void check_all_is_different(int begin, int end) {
        if (begin != end) {
            IdValuePair idValuePair = this.pairs[begin];
            while (true) {
                IdValuePair prev = idValuePair;
                begin++;
                if (begin != end) {
    ...
    private static void heapify4(IdValuePair[] array, int offset, int size, int i, int comparator) {
        ...
        while (true) {
            ...
            if (new_i4 < size) {
                ...
                if (new_i1 < size) {
                    IdValuePair val12 = array[offset + new_i1];
                    if (new_i2 != size) {
                        IdValuePair val22 = array[offset + new_i2];
                        if (bigger(val22, val12, comparator)) {
                            val12 = val22;
                            new_i1 = new_i2;
                        }
                        if (new_i3 != size) {
                            IdValuePair val32 = array[offset + new_i3];
                            if (bigger(val32, val12, comparator)) {
                                val12 = val32;
                                new_i1 = new_i3;
                            }
                        }
                    }
                    if (bigger(val12, i_val, comparator)) {
                        array[offset + i] = val12;
                        array[offset + new_i1] = i_val;
                        return;
                    }
                    return;
