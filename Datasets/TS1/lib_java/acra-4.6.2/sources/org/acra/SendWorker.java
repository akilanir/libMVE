...
/* loaded from: acra-4.6.2.jar:org/acra/SendWorker.class */
    ...
    private void checkAndSendReports(Context context, boolean sendOnlySilentReports) {
        ...
        for (String curFileName : reportFiles) {
            if (!sendOnlySilentReports || this.fileNameParser.isSilent(curFileName)) {
                ...
                ACRA.log.i(ACRA.LOG_TAG, "Sending file " + curFileName);
                try {
                    ...
                    ACRA.log.e(ACRA.LOG_TAG, "Failed to load crash report for " + curFileName, e);
                    deleteFile(context, curFileName);
                } catch (RuntimeException e2) {
                    ACRA.log.e(ACRA.LOG_TAG, "Failed to send crash reports for " + curFileName, e2);
                    deleteFile(context, curFileName);
                } catch (ReportSenderException e3) {
                    ACRA.log.e(ACRA.LOG_TAG, "Failed to send crash report for " + curFileName, e3);
                }
                reportsSentCount++;
            }
        }
