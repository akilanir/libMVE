...
/* loaded from: com.google.android.exoplayer.exoplayer.r1.5.7.jar:com/google/android/exoplayer/dash/mpd/UrlTemplate.class */
public final class UrlTemplate {
    ...
    private static int parseTemplate(String template, String[] urlPieces, int[] identifiers, String[] identifierFormatTags) {
        ...
        while (templateIndex < template.length()) {
            ...
            if (dollarIndex == -1) {
                int i = identifierCount;
                urlPieces[i] = urlPieces[i] + template.substring(templateIndex);
                templateIndex = template.length();
            } else if (dollarIndex != templateIndex) {
                int i2 = identifierCount;
                urlPieces[i2] = urlPieces[i2] + template.substring(templateIndex, dollarIndex);
                templateIndex = dollarIndex;
            } else if (template.startsWith(ESCAPED_DOLLAR, templateIndex)) {
                int i3 = identifierCount;
                urlPieces[i3] = urlPieces[i3] + "$";
                templateIndex += 2;
            } else {
