...
@TargetApi(9)
/* loaded from: com.koushikdutta.async.androidasync.2.1.9.jar:com/koushikdutta/async/ByteBufferList.class */
public class ByteBufferList {
    ...
    public String peekString(Charset charset) {
        ...
        Iterator i$ = this.mBuffers.iterator();
        while (i$.hasNext()) {
    ...
    /* loaded from: com.koushikdutta.async.androidasync.2.1.9.jar:com/koushikdutta/async/ByteBufferList$Reclaimer.class */
    ...
    private static boolean reclaimedContains(ByteBuffer b) {
        Iterator i$ = reclaimed.iterator();
        while (i$.hasNext()) {
            ByteBuffer other = i$.next();
            if (other == b) {
                return true;
            }
        }
    ...
    public static void obtainArray(ByteBuffer[] arr, int size) {
        ...
        if (total < size) {
            int i2 = index;
            index++;
            arr[i2] = ByteBuffer.allocate(Math.max(8192, size - total));
        }
        for (int i3 = index; i3 < arr.length; i3++) {
            arr[i3] = EMPTY_BYTEBUFFER;
        }
    }
